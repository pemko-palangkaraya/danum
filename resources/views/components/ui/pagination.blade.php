    ...
    <div class="hidden items-center gap-1 sm:flex">
        ...
        @for ($page = 1; $page <= $paginator->lastPage(); $page++)
            <button
                ...
                class="inline-flex h-8 min-w-8 shrink-0 items-center justify-center rounded-lg border px-2 text-xs font-medium transition {{ $paginator->currentPage() === $page ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                {{ $page }}
            </button>
        @endfor
