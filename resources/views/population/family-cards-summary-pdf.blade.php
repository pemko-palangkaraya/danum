...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { font-family: DejaVu Sans, sans-serif; font-size: 6.5px; color: #111; margin: 0; }
        .header { border-bottom: 1.5px solid #111; padding-bottom: 6px; margin-bottom: 6px; }
        .title { font-size: 13px; font-weight: bold; text-transform: uppercase; }
        .subtitle { font-size: 8px; font-weight: bold; margin-top: 2px; }
        .meta { font-size: 6px; color: #555; margin-top: 2px; }
        .scope { float: right; text-align: right; font-size: 6px; line-height: 1.35; }
        .clearfix { clear: both; }

        .kpi { width: 100%; border-collapse: separate; border-spacing: 3px; margin: 0 -3px 4px; }
        .kpi td { width: 16.666%; border: 1px solid #222; padding: 5px 3px; text-align: center; vertical-align: middle; }
        .kpi .value { display: block; font-size: 11px; font-weight: bold; }
        .kpi .label { display: block; margin-top: 2px; font-size: 5.5px; font-weight: bold; text-transform: uppercase; }

        .grid { width: 100%; border-collapse: separate; border-spacing: 4px 0; margin: 0 -4px; table-layout: fixed; }
        .grid > tbody > tr > td { width: 33.333%; vertical-align: top; padding: 0 4px; }
        .section { margin-bottom: 5px; page-break-inside: avoid; }
        .section-title { background: #e9e9e9; border: 1px solid #888; border-bottom: 0; padding: 3px 4px; font-size: 6.5px; font-weight: bold; text-transform: uppercase; }
        table.data { width: 100%; border-collapse: collapse; table-layout: fixed; }
        .data th, .data td { border: 1px solid #aaa; padding: 2px 3px; line-height: 1.15; }
        .data th { background: #f5f5f5; font-weight: bold; text-align: left; }
        .data th:last-child, .data td:last-child { text-align: right; width: 27%; }
        .data .pct { width: 20%; }
        .data .num { text-align: right; }
        .muted { color: #666; }

        .mini { width: 100%; border-collapse: collapse; }
        .mini td { padding: 2px 3px; border-bottom: 1px solid #ccc; }
        .mini td:last-child { text-align: right; font-weight: bold; }

        .wide { margin-top: 4px; }
        .wide-grid { width: 100%; border-collapse: separate; border-spacing: 4px 0; margin: 0 -4px; table-layout: fixed; }
        .wide-grid td { width: 50%; vertical-align: top; padding: 0 4px; }

        .note { border-top: 1px solid #aaa; margin-top: 5px; padding-top: 4px; font-size: 5.5px; color: #555; line-height: 1.35; }
        .footer { margin-top: 3px; text-align: right; font-size: 5.5px; color: #777; }
    </style>
</head>
<body>
    <div class="header">
        ...
        <tr>
            <td>
                <div class="section">
                    ...
                    <table class="data">
                        <tr><th>Jenis kelamin</th><th class="pct">Jumlah</th><th class="pct">%</th></tr>
                        @foreach ($aggregate['gender'] as $label => $total)
                            <tr><td>{{ $label }}</td><td class="num">{{ number_format($total) }}</td><td class="num">{{ number_format($aggregate['gender_percentages'][$label] ?? 0, 1) }}%</td></tr>
                        @endforeach
                    </table>
                </div>
                ...
                <div class="section">
                    ...
                    <table class="data">
                        ...
                        @foreach ($aggregate['age_groups'] as $label => $total)
                            <tr><td>{{ $label }}</td><td class="num">{{ number_format($total) }}</td></tr>
                        @endforeach
                    </table>
                </div>
            ...
            <td>
                ...
                <div class="section">
                    ...
                    <table class="data">
                        <tr><td>KK hanya 1 orang</td><td class="num">{{ number_format($aggregate['single_member_families']) }}</td></tr>
                        <tr><td>KK dengan ≥5 anggota</td><td class="num">{{ number_format($aggregate['large_families']) }}</td></tr>
                        <tr><td>KK memiliki anak</td><td class="num">{{ number_format($aggregate['families_with_children']) }}</td></tr>
                        <tr><td>KK memiliki lansia</td><td class="num">{{ number_format($aggregate['families_with_elderly']) }}</td></tr>
                        <tr><td>WNI</td><td class="num">{{ number_format($aggregate['wni']) }}</td></tr>
                        <tr><td>WNA</td><td class="num">{{ number_format($aggregate['wna']) }}</td></tr>
                    </table>
                </div>
            </td>
            ...
            <td>
                ...
                <div class="section">
                    ...
                    <table class="data">
                        <tr><th>Kelurahan</th><th>KK</th><th>Penduduk</th></tr>
                        @foreach ($aggregate['kelurahan'] as $label => $values)
                            <tr><td>{{ $label }}</td><td class="num">{{ number_format($values['kk']) }}</td><td class="num">{{ number_format($values['penduduk']) }}</td></tr>
                        @endforeach
                    </table>
                </div>
                ...
                <div class="section">
                    ...
                    <table class="mini">
                        <tr><td>Minimum anggota</td><td>{{ number_format($aggregate['minimum_members']) }}</td></tr>
                        <tr><td>Maksimum anggota</td><td>{{ number_format($aggregate['maximum_members']) }}</td></tr>
                        <tr><td>Median anggota</td><td>{{ number_format($aggregate['median_members'], 2) }}</td></tr>
                        <tr><td>Rata-rata anggota</td><td>{{ number_format($aggregate['average_members'], 2) }}</td></tr>
                    </table>
                </div>
            </td>
        </tr>
