...
<head>
    ...
    <style>
        ...
        .footer { bottom: 0; color: #6b7280; font-size: 8pt; position: fixed; text-align: center; width: 100%; }
    </style>
</head>
<body>
    <header class="header">
        ...
        <div class="tenant-address">
            {{ implode(', ', array_filter([
                $letter->tenant->village,
                $letter->tenant->district,
                $letter->tenant->city,
                $letter->tenant->province,
            ])) }}
    ...
    <table class="meta">
        ...
        @if ($letter->recipient_address)
            <tr><td>Alamat</td><td>: {{ $letter->recipient_address }}</td></tr>
        @endif
    ...
    <footer class="footer">Dokumen ini dihasilkan oleh sistem Danum.</footer>
</body>
</html>
