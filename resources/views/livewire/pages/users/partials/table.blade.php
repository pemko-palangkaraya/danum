        ...
        <table class="min-w-full">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-slate-500">Pangkat</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($users as $user)
                    <tr class="hover:bg-slate-50/70">
                        ...
                        <td class="px-5 py-4 text-sm text-slate-600">{{ $user->employeeProfile?->nip ?: '-' }}</td>
                        <td class="px-5 py-4 text-sm text-slate-600">{{ $user->employeeProfile?->pangkat ?: '-' }}</td>
            ...
            <div class="flex items-center justify-between gap-3 p-4">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-1 text-xs text-slate-500">
                        {{ $user->employeeProfile?->nip ?: '-' }} · {{ $user->tenant?->name ?? 'System' }}
                    </div>
                    <div class="mt-1 text-xs text-slate-500">
                        {{ $user->employeeProfile?->pangkat ?: '-' }} · {{ $user->isSuperAdmin() ? 'Super Admin' : ($user->effectiveRole()?->name ?? '-') }}
                    </div>
