<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><h1 class="text-2xl font-semibold tracking-tight text-slate-900">Letter Type Management</h1><p class="mt-1 text-sm text-slate-500">Master jenis surat global. Template DOCX dan variabel dikelola Super Admin.</p></div>
    ...
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="border-b border-slate-100 px-6 py-5"><h2 class="text-lg font-semibold">{{ $editingId ? 'Edit Letter Type' : 'Add Letter Type' }}</h2><p class="mt-1 text-sm text-slate-500">Upload DOCX yang sudah berisi placeholder <code>{{ '{{variable}}' }}</code>. Sistem akan memeriksa semua variabel sebelum menyimpan.</p></div>
                <form wire:submit="save" class="space-y-5 p-6">
                    <div class="grid gap-5 sm:grid-cols-2"><div><label class="text-sm font-medium">Code</label><input wire:model="code" class="mt-1 w-full rounded-xl border-slate-200"><x-input-error :messages="$errors->get('code')" class="mt-1" /></div><div><label class="text-sm font-medium">Name</label><input wire:model="name" class="mt-1 w-full rounded-xl border-slate-200"><x-input-error :messages="$errors->get('name')" class="mt-1" /></div></div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4"><div class="flex items-center justify-between"><div><label class="text-sm font-semibold">Template DOCX</label><p class="mt-1 text-xs text-slate-500">Maksimal 10 MB.</p></div><input wire:model="template_file" type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" class="block max-w-xs text-sm"></div><x-input-error :messages="$errors->get('template_file')" class="mt-2" /></div>
                    <div class="rounded-xl border border-indigo-100 bg-indigo-50 p-4"><h3 class="text-sm font-semibold text-indigo-900">Variabel yang didukung</h3><div class="mt-3 grid gap-2 sm:grid-cols-2">@foreach($availableVariables as $variable => $label)<div class="rounded-lg bg-white px-3 py-2"><code class="text-xs font-semibold text-indigo-700">{{ '{{'.$variable.'}}' }}</code><p class="text-xs text-slate-500">{{ $label }}</p></div>@endforeach</div><p class="mt-3 text-xs text-indigo-700">Tenant User tidak mengetik template. Saat membuat surat, sistem akan membuat input berdasarkan variabel yang dipakai template.</p></div>
