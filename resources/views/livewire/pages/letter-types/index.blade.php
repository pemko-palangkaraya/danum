<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Master jenis surat global. Super Admin menentukan variabel dan template DOCX.</p>
        </div>
            ...
            @forelse ($letterTypes as $letterType)
                <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        ...
                        <p class="mt-2 text-xs text-slate-400">{{ count($letterType->variables ?? []) }} variabel · Template {{ $letterType->template_path ? 'DOCX' : 'belum diupload' }}</p>
                    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="border-b border-slate-100 px-6 py-5">
                    <h2 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Edit Letter Type' : 'Add Letter Type' }}</h2>
                    <p class="mt-1 text-sm text-slate-500">Tentukan variabel, lalu upload DOCX. Sistem mencocokkan placeholder DOCX dengan daftar variabel sebelum menyimpan.</p>
                </div>
                <form wire:submit="save" class="space-y-5 p-6">
                    ...
                    <div class="rounded-xl border border-indigo-100 bg-indigo-50 p-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-indigo-700">Masukkan nama variabel yang benar-benar dipakai di DOCX. Satu variabel per baris. Tidak perlu menulis <code>&#123;&#123;</code> dan <code>&#125;&#125;</code>.</p>
                        </div>
                        <textarea wire:model="variables_input" rows="7" placeholder="number&#10;recipient_name&#10;recipient_nik&#10;recipient_address&#10;subject&#10;tenant_name&#10;tenant_city&#10;tenant_head_name&#10;date" class="form-textarea mt-3 font-mono text-sm"></textarea>
                        @error('variables_input')<p class="mt-2 text-xs text-rose-600">{{ $message }}</p>@enderror
                        <div class="mt-3 rounded-lg bg-white p-3 text-xs text-indigo-800"><strong>Contoh:</strong> <code>recipient_name</code> akan dicocokkan dengan <code>&#123;&#123;recipient_name&#125;&#125;</code> di DOCX.</div>
                    </div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-500">Maksimal 10 MB. Placeholder <code>&#123;&#123;variable&#125;&#125;</code> di DOCX harus sama persis dengan daftar variabel.</p>
                            </div>
                            <input wire:model="template_file" type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" class="block w-full max-w-xs rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-slate-700">
                        </div>
                        @error('template_file')<p class="mt-2 text-xs text-rose-600">{{ $message }}</p>@enderror
                        <div class="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600">Saat disimpan, DANUM melakukan <strong>cross-check</strong>. Variabel yang hilang atau berlebih akan ditolak.</div>
                    </div>
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-100 pt-5">
                        ...
                        <button type="submit" class="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">Save Master Template</button>
                    </div>
                </form>
            </div>
        </div>
