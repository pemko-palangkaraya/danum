            ...
            @forelse ($letterTypes as $letterType)
                <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        ...
                        <p class="mt-2 text-xs text-slate-400">{{ count($letterType->variables ?? []) }} variabel · Template {{ $letterType->template_path ? 'DOCX tersedia' : 'belum tersedia' }} · Masa berlaku: {{ match($letterType->validity_period ?? 'none') { '1_week' => '1 minggu', '2_weeks' => '2 minggu', '1_month' => '1 bulan', '3_months' => '3 bulan', '6_months' => '6 bulan', '1_year' => '1 tahun', default => 'Tidak ada' } }}</p>
                    </div>
                    <div class="flex shrink-0 flex-wrap gap-2">
                        ...
                        @endif
                        <button wire:click="edit('{{ $letterType->id }}')" class="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Edit Master</button><button wire:click="delete('{{ $letterType->id }}')" wire:confirm="Hapus jenis surat ini?" class="rounded-lg border border-rose-200 px-3 py-2 text-sm font-medium text-rose-600 hover:bg-rose-50">Delete</button>
                    </div>
                </div>
            @empty
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
                ...
                <form wire:submit="save" class="space-y-5 p-6">
                    ...
                    <div class="rounded-xl border border-indigo-100 bg-indigo-50 p-4"><div><h3 class="text-sm font-semibold text-indigo-900">Variabel Template</h3><p class="mt-1 text-xs text-indigo-700">Daftar variabel yang digunakan oleh template. Satu variabel per baris.</p></div><textarea wire:model="variables_input" rows="7" placeholder="number&#10;recipient_name&#10;recipient_nik&#10;recipient_address&#10;subject&#10;tenant_name&#10;tenant_city&#10;tenant_head_name&#10;date" class="form-textarea mt-3 font-mono text-sm"></textarea>@error('variables_input')<p class="mt-2 text-xs text-rose-600">{{ $message }}</p>@enderror<div class="mt-3 rounded-lg bg-white p-3 text-xs text-indigo-800"><strong>Catatan:</strong> perubahan daftar variabel akan divalidasi terhadap template versi yang sedang digunakan.</div></div>
                    @if ($editingId)
                        <div class="rounded-xl border border-violet-100 bg-violet-50 p-4"><p class="text-sm font-semibold text-violet-900">Template DOCX dikelola melalui Kelola Versi</p><p class="mt-1 text-xs text-violet-700">Untuk mengganti format dokumen, kembali ke daftar jenis surat lalu pilih <strong>Kelola Versi</strong>. Versi baru akan memiliki periode berlaku dan catatan perubahan sendiri.</p></div>
                    @endif
