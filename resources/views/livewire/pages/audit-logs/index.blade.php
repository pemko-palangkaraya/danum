...
>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr class="text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                        <th class="px-4 py-3"><x-ui.table-sort-header column="created" label="Waktu" :sort-by="$sortBy" :sort-direction="$sortDirection" /></th>
                        <th class="px-4 py-3"><x-ui.table-sort-header column="actor" label="Actor" :sort-by="$sortBy" :sort-direction="$sortDirection" /></th>
                        <th class="px-4 py-3"><x-ui.table-sort-header column="tenant" label="Tenant" :sort-by="$sortBy" :sort-direction="$sortDirection" /></th>
                        <th class="px-4 py-3"><x-ui.table-sort-header column="action" label="Action" :sort-by="$sortBy" :sort-direction="$sortDirection" /></th>
                        <th class="px-4 py-3"><x-ui.table-sort-header column="object" label="Object" :sort-by="$sortBy" :sort-direction="$sortDirection" /></th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($logs as $log)
                        ...
                        <tr class="hover:bg-slate-50/70">
                            ...
                            <td class="max-w-44 px-4 py-3"><div class="truncate text-sm font-semibold text-slate-900" title="{{ $log->user?->name ?? 'System / Unknown' }}">{{ $log->user?->name ?? 'System / Unknown' }}</div>@if ($log->user?->email)<div class="mt-0.5 truncate text-xs text-slate-500" title="{{ $log->user->email }}">{{ $log->user->email }}</div>@endif</td>
                            <td class="max-w-40 px-4 py-3">@if ($log->tenant)<div class="truncate text-sm font-medium text-slate-800" title="{{ $log->tenant->name }}">{{ $log->tenant->name }}</div>@if ($log->tenant->code)<div class="truncate text-xs text-slate-500">{{ $log->tenant->code }}</div>@endif @else<span class="text-sm text-slate-400">Global</span>@endif</td>
                            ...
                            <td class="max-w-48 px-4 py-3"><div class="truncate text-sm font-medium text-slate-800" title="{{ $log->auditable_type }}">{{ class_basename($log->auditable_type ?? '—') }}</div><div class="mt-0.5 truncate font-mono text-xs text-slate-500" title="{{ $log->auditable_id ?? '—' }}">{{ $log->auditable_id ?? '—' }}</div></td>
                            <td class="whitespace-nowrap px-4 py-3 text-right"><button type="button" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:text-slate-900" x-on:click="detail = @js($auditDetail); open = true">View</button></td>
                        </tr>
                    @empty
                ...
                <div class="p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2"><span class="inline-flex max-w-full truncate rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{{ $log->action }}</span><span class="text-xs text-slate-400">{{ $log->created_at?->format('d M Y H:i') }}</span></div>
                        ...
                        <button type="button" class="shrink-0 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm" x-on:click="detail = @js($auditDetail); open = true">View</button>
                    </div>
                </div>
            @empty
    ...
    <div x-cloak x-show="open" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" x-on:click.self="open = false" role="dialog" aria-modal="true" aria-labelledby="audit-detail-title">
        <div x-show="open" x-transition class="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
                <div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><h2 id="audit-detail-title" class="text-lg font-bold text-slate-900">Detail Audit Log</h2><span class="inline-flex max-w-full truncate rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700" x-text="detail?.action"></span></div><p class="mt-1 text-xs text-slate-500" x-text="detail?.createdAt"></p></div>
            ...
            <div class="max-h-[calc(90vh-80px)] space-y-5 overflow-y-auto p-5">
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">Actor</p><p class="mt-1 text-sm font-semibold text-slate-800" x-text="detail?.actor || '—'"></p><p class="mt-0.5 truncate text-xs text-slate-500" x-text="detail?.actorEmail || ''"></p></div>
                    <div><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">Tenant</p><p class="mt-1 text-sm font-semibold text-slate-800" x-text="detail?.tenant || 'Global'"></p><p class="mt-0.5 text-xs text-slate-500" x-text="detail?.tenantCode || ''"></p></div>
                    <div><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">Object</p><p class="mt-1 text-sm font-semibold text-slate-800" x-text="detail?.object || '—'"></p><p class="mt-0.5 break-all font-mono text-xs text-slate-500" x-text="detail?.objectId || '—'"></p></div>
                    <div><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">IP</p><p class="mt-1 break-all font-mono text-sm font-semibold text-slate-800" x-text="detail?.ip || '—'"></p><p class="mt-0.5 text-xs text-slate-500">Audit ID: <span x-text="detail?.id || '—'"></span></p></div>
                </div>
                <div class="grid gap-4 lg:grid-cols-2">
                    <div><div class="mb-2 flex items-center justify-between"><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">Before</p></div><pre class="max-h-80 overflow-auto rounded-xl bg-slate-950 p-4 text-xs leading-5 text-slate-100" x-text="JSON.stringify(detail?.oldValues ?? {}, null, 2)"></pre></div>
                    <div><div class="mb-2 flex items-center justify-between"><p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">After</p></div><pre class="max-h-80 overflow-auto rounded-xl bg-slate-950 p-4 text-xs leading-5 text-slate-100" x-text="JSON.stringify(detail?.newValues ?? {}, null, 2)"></pre></div>
                </div>
            </div>
        </div>
    </div>
