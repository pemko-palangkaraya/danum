<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-3">
                ...
                <span @class([
                    ...
                    'bg-amber-100 text-amber-800' => $status === 'validated',
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('outgoing-letters.pdf', $letter->id) }}" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">{{ $status === 'issued' ? 'View PDF' : 'Preview PDF' }}</a>
            @if ($status === 'issued')
    ...
    @if ($status === 'draft')
        <div class="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><strong>Draft.</strong> Surat masih dapat diedit. Preview diberi watermark dan belum memiliki QR/TTE resmi.</div>
    @elseif ($status === 'validated')
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Validated.</strong> Data surat sudah lolos validasi dan siap diterbitkan. QR/TTE resmi belum aktif.</div>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                    ...
                    @foreach ($contentParts as $part)
                        @if (preg_match('/^\{\{\s*tte\s*\}\}$/i', trim($part)))
                            @if ($status === 'issued' && $verificationQrCode)
                                <div class="my-4 flex flex-col items-center text-center font-sans text-xs text-slate-500"><img src="{{ $verificationQrCode }}" alt="QR TTE / verifikasi surat" class="h-28 w-28"><span class="mt-1">TTE / verifikasi dokumen</span></div>
                            @endif
                        @else
                            {!! nl2br(e($part)) !!}
                        @endif
        ...
        <aside class="h-fit space-y-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <dl class="mt-4 space-y-3 text-sm"><div><dt class="text-slate-400">Number</dt><dd class="font-medium text-slate-800">{{ $letter->number }}</dd></div><div><dt class="text-slate-400">Recipient</dt><dd class="font-medium text-slate-800">{{ $letter->recipient_name }}</dd></div><div><dt class="text-slate-400">Address</dt><dd class="text-slate-700">{{ $letter->recipient_address ?: '—' }}</dd></div><div><dt class="text-slate-400">Letter Date</dt><dd class="font-medium text-slate-800">{{ optional($letter->letter_date)->translatedFormat('d F Y') ?? '—' }}</dd></div></dl>
            </div>

            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between"><h2 class="font-semibold text-slate-900">Riwayat Surat</h2><span class="text-xs text-slate-400">{{ $history->count() }} event</span></div>
                <div class="mt-5 space-y-5">
                    @forelse ($history as $event)
                        <div class="relative pl-6">
                            @if (!$loop->last)<span class="absolute left-[5px] top-3 h-full w-px bg-slate-200"></span>@endif
                            <span class="absolute left-0 top-1.5 h-3 w-3 rounded-full border-2 border-white bg-slate-400 shadow"></span>
                            <div class="text-sm font-semibold text-slate-800">{{ ucfirst(str_replace('_', ' ', $event->action)) }}</div>
                            <div class="mt-0.5 text-xs text-slate-500">{{ $event->created_at?->translatedFormat('d F Y, H:i') }} · {{ $event->changedBy?->name ?? 'System' }}</div>
                        </div>
                    @empty
