    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-3">
                ...
                <span @class(['inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide','bg-slate-100 text-slate-700' => $status === 'draft','bg-amber-100 text-amber-800' => $status === 'validated','bg-emerald-100 text-emerald-800' => $status === 'issued','bg-red-100 text-red-800' => $status === 'cancelled'])>{{ $status }}</span>
            </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if ($status === 'issued')<a href="{{ route('outgoing-letters.pdf', ['id' => $letter->id, 'download' => 1]) }}" class="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800">Download Issued PDF</a>@endif
        ...
        <aside class="h-fit space-y-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    ...
                    @if($letter->validator_name)<div><dt class="text-slate-400">Validator</dt><dd class="font-medium text-slate-800">{{ $letter->validator_name }}</dd><dd class="text-xs text-slate-500">{{ $letter->validator_title }}</dd></div>@endif
                    @if($letter->signer_name)<div><dt class="text-slate-400">Penanda Tangan</dt><dd class="font-medium text-slate-800">{{ $letter->signer_name }}</dd><dd class="text-xs text-slate-500">{{ $letter->signer_title }}</dd></div>@endif
                </dl>
            </div>
