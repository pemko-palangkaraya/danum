    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
            ...
            <div class="mx-auto min-h-[297mm] max-w-[210mm] bg-white px-4 py-8 text-[14px] leading-7 text-slate-900 print:min-h-0 print:max-w-none print:px-0 print:py-0">
                @if ($letter->tenant?->letterheadUrl())
                    <div class="mb-7 border-b border-slate-300 pb-3">
                        <img src="{{ $letter->tenant->letterheadUrl() }}" alt="Kop surat {{ $letter->tenant->name }}" class="mx-auto max-h-32 w-full object-contain">
                    </div>
                @else
                    <header class="mb-7 border-b-4 border-slate-900 pb-3 text-center">
                        <div class="text-lg font-bold uppercase tracking-tight">{{ $letter->tenant?->name }}</div>
                        <div class="text-xs leading-5 text-slate-500">
                            ...
                            @if ($letter->tenant?->phone) · {{ $letter->tenant->phone }} @endif
                        </div>
                    </header>
                @endif

                <section class="mb-7 text-center">
                    <h2 class="text-base font-bold uppercase underline">{{ $letter->letterType?->name }}</h2>
                    <div class="text-sm">Nomor: {{ $letter->number }}</div>
                </section>

                <dl class="mb-7 ml-4 grid grid-cols-[75px_1fr] gap-y-1 text-sm">
                    <dt>Tujuan</dt><dd>: {{ $letter->recipient_name }}</dd>
                    @if ($letter->recipient_address)<dt>Alamat</dt><dd>: {{ $letter->recipient_address }}</dd>@endif
                    <dt>Perihal</dt><dd>: {{ $letter->subject }}</dd>
                </dl>
        ...
        <aside class="h-fit space-y-4 print:hidden">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                <h2 class="font-semibold text-slate-900">Kop Surat</h2>
                <p class="mt-1 text-sm text-slate-600">Tampilan surat mengikuti kop organisasi yang aktif.</p>
                @if ($letter->tenant?->letterheadUrl())
                    <span class="mt-3 inline-flex rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">Kop aktif</span>
                @else
                    <span class="mt-3 inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700">Kop default</span>
                @endif
            </div>
