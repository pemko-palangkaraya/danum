<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-3">
                ...
                <span @class(['inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide','bg-blue-100 text-blue-800' => $status === 'draft' && $submitted,'bg-slate-100 text-slate-700' => $status === 'draft' && ! $submitted,'bg-amber-100 text-amber-800' => $status === 'validated','bg-emerald-100 text-emerald-800' => $status === 'issued','bg-red-100 text-red-800' => $status === 'cancelled'])>{{ $status === 'draft' && $submitted ? 'Menunggu Verifikasi' : $status }}</span>
            </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside class="h-fit space-y-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="font-semibold text-slate-900">Letter Details</h2><dl class="mt-4 space-y-3 text-sm"><div><dt class="text-slate-400">Number</dt><dd class="font-medium text-slate-800">{{ $letter->number }}</dd></div><div><dt class="text-slate-400">Recipient</dt><dd class="font-medium text-slate-800">{{ $letter->recipient_name }}</dd></div><div><dt class="text-slate-400">Address</dt><dd class="text-slate-700">{{ $letter->recipient_address ?: '—' }}</dd></div><div><dt class="text-slate-400">Letter Date</dt><dd class="font-medium text-slate-800">{{ optional($letter->letter_date)->translatedFormat('d F Y') ?? '—' }}</dd></div>@if($letter->validator_name)<div><dt class="text-slate-400">Verifikator</dt><dd class="font-medium text-slate-800">{{ $letter->validator_name }}</dd><dd class="text-xs text-slate-500">{{ $letter->validator_title }}</dd></div>@endif @if($letter->signer_name)<div><dt class="text-slate-400">Penanda Tangan</dt><dd class="font-medium text-slate-800">{{ $letter->signer_name }}</dd><dd class="text-xs text-slate-500">{{ $letter->signer_title }}</dd></div>@endif</dl></div>
