    ...
    <div class="min-w-0">
        <div class="flex flex-wrap items-center gap-2">
            ...
            <span @class([
                ...
                'withdrawn' => 'Ditarik', 'expired' => 'Kedaluwarsa', 'issued' => 'Diterbitkan', 'validated' => 'Tervalidasi', 'cancelled' => 'Dibatalkan', default => 'Draft'
            }) }}</span>
            @if($letter->trashed())<span class="rounded-full bg-rose-100 px-2 py-0.5 text-xs font-medium text-rose-700">Dihapus</span>@endif
        </div>
        ...
        @if(! $letter->trashed())
            ...
            @unless($isSuperAdmin)
                ...
                @if($letter->status->value === 'validated' && (int) $letter->signer_user_id === (int) auth()->id())
                    @can('issue', $letter)<button wire:click="issue('{{ $letter->id }}')" class="rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white hover:bg-slate-800">Terbitkan</button>@endcan
                    @can('reject', $letter)<button wire:click="openReject('{{ $letter->id }}')" class="rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-50">Tolak</button>@endcan
                @endif
            @endunless
            @if($letter->status->value === 'issued')<a href="{{ route('verification.show', $letter->verification_token) }}" target="_blank" class="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-medium text-emerald-700 hover:bg-emerald-50">Verifikasi</a>@endif
        @elseif($isSuperAdmin)
