<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="cancelForm">
    <div class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-xl">
        <div class="border-b border-slate-100 px-6 py-5">
            <h2 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Edit Draft Surat' : 'Buat Surat Keluar' }}</h2>
        ...
        <form wire:submit="save" class="space-y-5 p-6">
            <div class="grid gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    @foreach([
                        ['validator_position_id','Verifikator',$validatorPositions,'Belum ada verifikator tersedia','Pilih verifikator'],
                        ['signer_position_id','Penanda Tangan',$signerPositions,'Pilih pejabat penanda tangan','Pilih pejabat penanda tangan']
                    ] as [$field,$label,$positions,$empty,$placeholder])
                        <div>
                            ...
                            <select wire:model="{{ $field }}" class="form-select mt-1">
                                ...
                                @foreach($positions as $position)
                                    <option value="{{ $position->id }}">{{ $position->name }} — {{ $position->holders->first()?->user?->name ?? 'Belum ditetapkan' }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        @foreach($variables as $variable)
                            @php
                                ...
                                $definition = \App\Support\LetterVariableSchema::parseRepeater($variable);
                                ...
                                $schema = \App\Models\LetterVariableDefinition::query()
                                    ...
                                    ->first();
                                $label = $schema?->label ?? $variableLabels[$variable] ?? ucwords(str_replace('_', ' ', $variable));
                                $isCitizenLookup = in_array($variable, ['recipient_nik', 'nik'], true);
                                $readOnly = ! $isCitizenLookup && ($this->isReadOnlyVariable($variable) || (bool) $schema?->readonly);
                                ...
                                $dateVariable = $variable === 'tanggal_meninggal' || $inputType === 'date' || (bool) preg_match('/(^|_)date$/i', $variable);
                                ...
                                <div class="{{ $wide ? 'sm:col-span-2' : '' }}">
                                    ...
                                    @elseif($inputType === 'checkbox')
                                        <label class="mt-2 flex items-center gap-2 text-sm text-slate-700">
                                            <input type="checkbox" wire:model="variableValues.{{ $variable }}" class="rounded">
                                            {{ $label }}
                                        </label>
            ...
            <div class="flex items-center justify-between gap-2 border-t border-slate-100 pt-5">
                ...
                <div class="flex gap-2">
                    <button type="button" wire:click="cancelForm" class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>
                    <button type="submit" class="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">{{ $editingId ? 'Simpan Perubahan' : 'Simpan Draft' }}</button>
                </div>
            </div>
        </form>
    </div>
