    ...
    @if($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="border-b border-slate-100 px-6 py-5"><h2 class="text-lg font-semibold text-slate-900">Create Outgoing Letter</h2><p class="mt-1 text-sm text-slate-500">Pilih jenis surat dan pejabat penanda tangan. Field dibuat otomatis dari placeholder template.</p></div>
                <form wire:submit="save" class="space-y-5 p-6">
                    ...
                    @if($variables)
                        <div class="rounded-xl border border-slate-200 bg-white p-4"><div><h3 class="text-sm font-semibold text-slate-900">Data Surat</h3><p class="mt-1 text-xs text-slate-500">Nama dan jabatan penanda tangan dikelola sistem dari jabatan yang dipilih.</p></div>
