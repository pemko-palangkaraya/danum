<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Buat, lihat, validasi, terbitkan, dan verifikasi surat keluar.</p>
        </div>
                ...
                <div class="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{{ $letter->status->value }}</span>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @if($letter->status->value === 'draft')
                            <button wire:click="validateLetter('{{ $letter->id }}')" class="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Validate</button>
                        @endif
                        @if($letter->status->value === 'validated')
                            <button wire:click="issue('{{ $letter->id }}')" class="rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white hover:bg-slate-800">Issue</button>
                        @endif
                        @if($letter->status->value === 'issued')
                            <a href="{{ route('verification.show', $letter->verification_token) }}" target="_blank" class="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-medium text-emerald-700 hover:bg-emerald-50">Verify</a>
                        @endif
                    </div>
                </div>
            @empty
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="border-b border-slate-100 px-6 py-5">
                    <h2 class="text-lg font-semibold text-slate-900">Create Outgoing Letter</h2>
                    <p class="mt-1 text-sm text-slate-500">Pilih master surat. Field dibuat otomatis dari placeholder yang digunakan di template DOCX.</p>
                </div>

                <form wire:submit="save" class="space-y-5 p-6">
                    <div>
                        <label class="text-sm font-medium text-slate-700">Letter Type</label>
                        ...
                        @error('letter_type_id')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                    </div>
                    ...
                    @if($variables)
                        <div class="rounded-xl border border-slate-200 bg-white p-4">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-500">Hanya placeholder yang benar-benar dipakai template yang ditampilkan.</p>
                            </div>
                            ...
                            <div class="mt-4 grid gap-4 sm:grid-cols-2">
                                @foreach($variables as $variable)
                                    @php
                                        ...
                                        $systemVariable = in_array($variable, ['tenant_name','tenant_city','tenant_district','tenant_village','tenant_province','tenant_address','tenant_phone','tenant_email','tenant_head_name','tenant_head_title','date'], true);
                                    ...
                                    <div class="{{ $wide ? 'sm:col-span-2' : '' }}">
                                        <div class="flex items-center justify-between gap-3">
                                            ...
                                            @if($systemVariable)
                                                <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500">Otomatis</span>
                                            @endif
                                        </div>

                                        @if($variable === 'recipient_address' || $variable === 'tenant_address')
                                        ...
                                        @if($systemVariable)
                                            <p class="mt-1 text-xs text-slate-400">Diambil otomatis dari data organisasi / sistem.</p>
                                        @endif
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-100 pt-5">
                        ...
                        <button type="submit" class="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">Create Draft</button>
                    </div>
                </form>
            </div>
        </div>
