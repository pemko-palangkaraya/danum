    ...
    @if($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-xl">
                <div class="border-b border-slate-100 px-6 py-5"><h2 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Edit Draft Surat' : 'Create Outgoing Letter' }}</h2><p class="mt-1 text-sm text-slate-500">{{ $editingId ? 'Periksa dan perbarui data sebelum dikirim untuk verifikasi.' : 'Isi data surat, lalu gunakan Preview untuk pemeriksaan akhir sebelum Submit.' }}</p></div>
                <form wire:submit="save" class="space-y-5 p-6">
                    <div class="grid gap-4">
                        <div><label class="text-sm font-medium text-slate-700">Jenis Surat</label><select wire:model.live="letter_type_id" class="form-select mt-1"><option value="">Pilih jenis surat</option>@foreach($letterTypes as $type)<option value="{{ $type->id }}">{{ $type->code }} — {{ $type->name }}</option>@endforeach</select>@error('letter_type_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div><label class="text-sm font-medium text-slate-700">Verifikator</label><select wire:model="validator_position_id" class="form-select mt-1"><option value="">{{ $validatorPositions->isEmpty() ? 'Belum ada verifikator tersedia' : 'Pilih verifikator' }}</option>@foreach($validatorPositions as $position)<option value="{{ $position->id }}">{{ $position->name }} — {{ $position->holders->first()?->user?->name ?? 'Belum ditetapkan' }}</option>@endforeach</select>@error('validator_position_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                            <div><label class="text-sm font-medium text-slate-700">Penanda Tangan</label><select wire:model="signer_position_id" class="form-select mt-1"><option value="">Pilih pejabat penanda tangan</option>@foreach($signerPositions as $position)<option value="{{ $position->id }}">{{ $position->name }} — {{ $position->holders->first()?->user?->name ?? 'Belum ditetapkan' }}</option>@endforeach</select>@error('signer_position_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        </div>
                    </div>
                    @if($variables)
