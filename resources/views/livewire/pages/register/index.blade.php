<div class="space-y-6">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th class="px-4 py-3">No. Register</th><th class="px-4 py-3">Nomor Surat</th><th class="px-4 py-3">Tanggal</th><th class="px-4 py-3">Perihal</th><th class="px-4 py-3">Tujuan</th><th class="px-4 py-3">Sumber</th><th class="px-4 py-3"></th></tr></thead><tbody class="divide-y divide-slate-100">
            @forelse($entries as $entry)
                <tr class="hover:bg-slate-50"><td class="whitespace-nowrap px-4 py-3 font-medium">{{ $entry->register_number }}</td><td class="whitespace-nowrap px-4 py-3 font-medium text-slate-900">{{ $entry->letter_number }}</td><td class="whitespace-nowrap px-4 py-3 text-slate-600">{{ $entry->letter_date?->format('d/m/Y') }}</td><td class="max-w-xs px-4 py-3 text-slate-700">{{ $entry->subject }}</td><td class="max-w-xs px-4 py-3 text-slate-600">{{ $entry->recipient_name }}</td><td class="px-4 py-3"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium {{ $entry->source->value === 'manual' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-700' }}">{{ strtoupper($entry->source->value) }}</span></td><td class="whitespace-nowrap px-4 py-3 text-right">@if($entry->source->value === 'manual' && auth()->user()->hasPermission('outgoing-letters.update'))<button type="button" wire:click="editManual('{{ $entry->id }}')" class="text-xs font-medium text-slate-600 hover:text-slate-900">Koreksi</button>@endif</td></tr>
            @empty
