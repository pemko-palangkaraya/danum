...
new #[Layout('layouts.app')] class extends Component {
    ...
    public ?int $source_order = null;
    public string $number_format = '{{number}}/{{classification_code}}/{{tenant_code}}/{{month_roman}}/{{year}}';
    ...
    public function openCreate(): void
    {
        ...
        $this->source_order = ((int) LetterClassification::query()->max('source_order')) + 1;
        $this->showForm = true;
    }
    ...
    public function openEdit(string $id): void
    {
        ...
        $this->source = (string) $classification->source;
        $this->source_order = $classification->source_order;
    ...
    public function save(AuditLogService $auditLogService): void
    {
        ...
        Validator::make(
            ...
            [
                ...
                'source_order' => ['nullable', 'integer', 'min:1', 'max:4294967295', Rule::unique('letter_classifications', 'source_order')->ignore($this->editingId)],
                'number_format' => [
        ...
        $old = $classification->exists ? [
            ...
            'source_order' => $classification->source_order,
        ...
        $classification->fill([
            ...
            'source' => filled($this->source) ? trim($this->source) : null,
            'source_order' => $this->source_order,
    ...
    public function resetForm(): void
    {
        ...
        $this->number_format = '{{number}}/{{classification_code}}/{{tenant_code}}/{{month_roman}}/{{year}}';
    ...
    public function classifications()
    {
        return LetterClassification::query()
            ->when($this->search !== '', fn ($q) => $q->where(fn ($q) => $q
                ...
                ->orWhere('source', 'ilike', '%'.$this->search.'%')))
...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Master klasifikasi sekaligus sumber dan aturan format nomor surat.</p>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <input wire:model.live.debounce.300ms="search" type="search" placeholder="Cari kode, klasifikasi, atau sumber..." class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-100 sm:w-80">
        </div>
                ...
                <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        ...
                        @if($classification->source)
                            <p class="mt-1 text-xs text-slate-400">Sumber: {{ $classification->source }}</p>
                        @endif
                        <p class="mt-2 font-mono text-xs text-indigo-700">{{ $classification->number_format }}</p>
                        <p class="mt-1 text-xs text-slate-400">Nomor {{ $classification->number_padding }} digit · Urutan {{ $classification->sort_order }}@if($classification->source_order) · Sumber #{{ $classification->source_order }}@endif</p>
                    </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            <label for="classification-per-page" class="text-xs text-slate-500">Show</label>
                            <select id="classification-per-page" wire:model.live="perPage" class="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs text-slate-700"><option value="5">5</option><option value="10">10</option><option value="25">25</option><option value="50">50</option></select>
                        </div>
                        <p class="text-xs text-slate-500">Showing {{ $classifications->firstItem() }} – {{ $classifications->lastItem() }} of {{ $classifications->total() }} classifications</p>
                    </div>
                    <x-ui.pagination :paginator="$classifications" />
                </div>
            </div>
        @endif
    </div>
                ...
                <form wire:submit="save" class="space-y-5 p-6">
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <input wire:model="code" class="form-control mt-1" placeholder="400.10">
                            <p class="mt-1 text-xs text-slate-400">Boleh sama jika memang terdapat kode duplikat pada sumber resmi.</p>
                            @error('code')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="text-sm font-medium text-slate-700">Nama</label>
                            <input wire:model="name" class="form-control mt-1" placeholder="Kesejahteraan Rakyat">
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="2" class="form-textarea mt-1" placeholder="Penjelasan singkat tentang klasifikasi..."></textarea>
                        @error('description')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                    </div>

                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            <label class="text-sm font-medium text-slate-700">Sumber</label>
                            <input wire:model="source" class="form-control mt-1" placeholder="Permendagri 83 Tahun 2022">
                            <p class="mt-1 text-xs text-slate-400">Nama dokumen/peraturan sumber klasifikasi.</p>
                            @error('source')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="text-sm font-medium text-slate-700">Urutan Sumber</label>
                            <input wire:model="source_order" type="number" min="1" class="form-control mt-1" placeholder="1">
                            <p class="mt-1 text-xs text-slate-400">Nomor urut baris pada sumber resmi. Kosongkan jika tidak ada.</p>
                            @error('source_order')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            <label class="text-sm font-medium text-slate-700">Jumlah digit nomor</label>
                            <input wire:model="number_padding" type="number" min="1" max="12" class="form-control mt-1">
                            @error('number_padding')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                    ...
                    <label class="flex items-center gap-3 text-sm font-medium text-slate-700">
                        <input wire:model="is_active" type="checkbox" class="rounded border-slate-300"> Klasifikasi aktif dan dapat dipilih oleh jenis surat
                    </label>

                    <div class="flex justify-end gap-2 border-t border-slate-100 pt-5">
                        <button type="button" wire:click="resetForm" class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>
                        <button type="submit" class="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
