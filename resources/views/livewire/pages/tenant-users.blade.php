...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function mount(): void { abort_unless(auth()->user()?->isTenantAdmin(), 403); $this->authorize('viewAny', User::class); }

    public function availableCustomRoles()
    {
        return Role::query()->where('is_system', false)->where('is_active', true)->where(function ($query) {
            $query->where('scope', 'global')->orWhere(fn ($tenant) => $tenant->where('scope', 'tenant')->where('tenant_id', auth()->user()->tenant_id));
    ...
    {
        $user=User::query()->where('tenant_id',auth()->user()->tenant_id)->findOrFail($id); $this->authorize('update',$user); $this->editingUserId=$user->id; $this->name=$user->name; $this->nip=(string)($user->nip??''); $this->email=$user->email; $this->password=''; $this->status=$user->status->value; $this->customRoleId=$user->custom_role_id?(string)$user->custom_role_id:null; $this->resetValidation(); $this->showForm=true;
    }

    public function openSignerPin(int $id): void
    {
        $user=User::query()->where('tenant_id',auth()->user()->tenant_id)->where('role',UserRole::TENANT_USER->value)->findOrFail($id); $this->authorize('update',$user); $this->signerPinUserId=$user->id; $this->signerPinUserName=$user->name; $this->signerPin=''; $this->signerPinConfirmation=''; $this->resetValidation(['signerPin','signerPinConfirmation']); $this->showSignerPin=true;
    }

    public function saveSignerPin(SignerPinService $pinService, AuditLogService $auditLogService): void
    {
        $user=User::query()->where('tenant_id',auth()->user()->tenant_id)->where('role',UserRole::TENANT_USER->value)->findOrFail($this->signerPinUserId); $this->authorize('update',$user); $validated=Validator::make(['signerPin'=>$this->signerPin,'signerPinConfirmation'=>$this->signerPinConfirmation],['signerPin'=>['required','digits:6'],'signerPinConfirmation'=>['required','same:signerPin']],['signerPin.required'=>'PIN wajib diisi.','signerPin.digits'=>'PIN harus terdiri dari 6 digit.','signerPinConfirmation.required'=>'Konfirmasi PIN wajib diisi.','signerPinConfirmation.same'=>'Konfirmasi PIN tidak sama.'])->validate(); $pinService->set($user,$validated['signerPin']); $auditLogService->record(action:'signer_pin.updated',user:auth()->user(),auditable:$user,newValues:['configured'=>true],tenantId:$user->tenant_id); $this->closeSignerPin(); $this->dispatch('toast',type:'success',message:'PIN tanda tangan berhasil disimpan.');
    }

    public function closeSignerPin(): void { $this->showSignerPin=false;$this->signerPinUserId=null;$this->signerPinUserName='';$this->signerPin='';$this->signerPinConfirmation='';$this->resetValidation(['signerPin','signerPinConfirmation']); }

    public function save(UserService $userService): void
    {
        $tenantId=(string)auth()->user()->tenant_id;
        $role=$this->customRoleId ? Role::query()->whereKey((int)$this->customRoleId)->where('is_system',false)->where('is_active',true)->where(function($q)use($tenantId){$q->where('scope','global')->orWhere(fn($t)=>$t->where('scope','tenant')->where('tenant_id',$tenantId));})->firstOrFail() : null;
        $data=['name'=>$this->name,'nip'=>$this->nip,'email'=>$this->email,'password'=>$this->password,'role'=>UserRole::TENANT_USER->value,'custom_role_id'=>$role?->id,'tenant_id'=>$tenantId,'status'=>$this->status];
        if($this->editingUserId){
            $user=User::query()->where('tenant_id',$tenantId)->findOrFail($this->editingUserId); $this->authorize('update',$user); if($this->password==='')unset($data['password']); $validated=Validator::make($data,array_merge((new UpdateUserRequest())->rules(),['custom_role_id'=>['nullable','integer','exists:roles,id']]))->validate(); if(isset($validated['password']))$validated['password']=Hash::make($validated['password']); $userService->update($user,$validated);
        }else{
            $this->authorize('create',User::class); $validated=Validator::make($data,array_merge((new StoreUserRequest())->rules(),['custom_role_id'=>['nullable','integer','exists:roles,id']]))->validate(); $validated['password']=Hash::make($validated['password']); $userService->create($validated);
        }
        $this->resetForm(); $this->dispatch('toast',type:'success',message:'Tenant user berhasil disimpan.');
    }

    public function toggleStatus(int $id, UserService $userService): void { $user=User::query()->where('tenant_id',auth()->user()->tenant_id)->findOrFail($id); $this->authorize('update',$user); $userService->update($user,['status'=>$user->status===UserStatus::ACTIVE?UserStatus::INACTIVE:UserStatus::ACTIVE]); $this->dispatch('toast',type:'success',message:'Status user diperbarui.'); }
    public function resetForm(): void { $this->showForm=false;$this->editingUserId=null;$this->name='';$this->nip='';$this->email='';$this->password='';$this->status=UserStatus::ACTIVE->value;$this->customRoleId=null;$this->resetValidation(); }
    public function with(): array { $tenantId=auth()->user()->tenant_id; return ['users'=>User::query()->where('tenant_id',$tenantId)->with('customRole')->orderBy('name')->get(),'tenantName'=>auth()->user()->tenant?->name??'Organisasi']; }
};
?>

<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p class="text-sm text-slate-500">Administration</p><h1 class="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Tenant Users</h1><p class="mt-1 text-sm text-slate-500">Kelola pengguna {{ $tenantName }} dan custom role mereka.</p></div><button type="button" wire:click="create" class="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">+ Add User</button></div>
    @if ($showForm)<div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 class="text-sm font-semibold text-slate-900">{{ $editingUserId?'Edit User':'Add User' }}</h2><div class="mt-5 grid gap-5 sm:grid-cols-2">
