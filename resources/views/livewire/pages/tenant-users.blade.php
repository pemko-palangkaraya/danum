...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function edit(int $id): void
    {
        $user = User::query()
            ...
            ->where('role', UserRole::TENANT_USER->value)
            ->findOrFail($id);
    ...
    {
        // Tenant Admin may manage Tenant Users only.
        $this->roleSelection = UserRole::TENANT_USER->value;
    }
    ...
    public function saveSignerPin(SignerPinService $pinService, AuditLogService $auditLogService): void
    {
        $user = User::query()
            ->where('tenant_id', auth()->user()->tenant_id)
            ->where('role', UserRole::TENANT_USER->value)
            ->findOrFail($this->signerPinUserId);
    ...
    public function save(UserService $userService): void
    {
        ...
        // This screen is intentionally limited to the tenant_user system role.
        $data = [
            'name' => $this->name,
            'nip' => $this->nip,
            'email' => $this->email,
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div class="mt-5 grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500">Tenant Admin hanya dapat mengelola user dengan role Tenant User. Role administrator dan custom role dikelola melalui RBAC/Super Admin.</p>
                </div>
                <div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-slate-700">Tenant User</td>
                            <td class="px-6 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $user->status === UserStatus::ACTIVE ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $user->status->value }}</span></td>
        ...
        @forelse ($users as $user)
            <div class="flex items-center justify-between gap-3 p-4">
                ...
                <div class="min-w-0 max-w-[42%] text-right">
                    <div class="truncate text-sm font-medium text-slate-800">Tenant User</div>
                    <div class="mt-1"><span class="rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $user->status === UserStatus::ACTIVE ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $user->status->value }}</span></div>
                </div>
                <div class="shrink-0"><x-ui.user-actions :user="$user" /></div>
            </div>
        @empty
            <div class="px-4 py-10 text-center text-sm text-slate-500">Belum ada Tenant User.</div>
        @endforelse
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="closeSignerPin">
            <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <button type="button" wire:click="closeSignerPin" class="rounded-lg px-2 py-1 text-slate-400 hover:bg-slate-100" aria-label="Tutup">✕</button>
                </div>
                <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800">PIN ini berbeda dari password login.</div>
                ...
                <div class="mt-6 flex justify-end gap-2"><button type="button" wire:click="closeSignerPin" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">Batal</button><button type="button" wire:click="saveSignerPin" class="rounded-xl bg-slate-900 px-5 py-2 text-sm font-semibold text-white">Simpan PIN</button></div>
            </div>
        </div>
    @endif
</div>
