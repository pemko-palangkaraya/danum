<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Kelola master jabatan dan riwayat pejabat yang berwenang.</p>
        </div>
        @if($isSuperAdmin)
                    ...
                    @forelse ($positions as $position)
                        ...
                        <tr>
                            ...
                            <td class="px-5 py-4 text-slate-700">
                                @if($holder?->user)
                                    <div class="font-medium">{{ $holder->user->name }}</div><div class="text-xs text-slate-500">Mulai {{ $holder->started_at?->format('d M Y') }}</div>
                                @else <span class="text-slate-400">Belum ditentukan</span> @endif
                            </td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    <button type="button" wire:click="showHistoryFor('{{ $position->id }}')" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">View</button>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showForm', false)">
            <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
                ...
                <div class="mt-5 space-y-4">
                    ...
                    @endif
                    <div><label class="text-sm font-medium text-slate-700">Kode Jabatan</label><input wire:model="code" type="text" placeholder="Contoh: LURAH" class="form-control mt-1">@error('code')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                    <div><label class="text-sm font-medium text-slate-700">Nama Jabatan</label><input wire:model="name" type="text" placeholder="Contoh: Lurah" class="form-control mt-1">@error('name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                    <div><label class="text-sm font-medium text-slate-700">Deskripsi <span class="font-normal text-slate-400">(opsional)</span></label><textarea wire:model="description" rows="3" class="form-textarea mt-1"></textarea></div>
                    <div><label class="text-sm font-medium text-slate-700">Status</label><select wire:model="status" class="form-select mt-1"><option value="active">Aktif</option><option value="inactive">Tidak Aktif</option></select></div>
                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 p-4"><input wire:model="can_sign" type="checkbox" class="mt-0.5 rounded border-slate-300"><span><span class="block text-sm font-semibold">Jabatan dapat menandatangani</span><span class="block text-xs text-slate-500">Pemegang aktif jabatan ini dapat dipilih sebagai penanda tangan.</span></span></label>
                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 p-4"><input wire:model="can_validate" type="checkbox" class="mt-0.5 rounded border-slate-300"><span><span class="block text-sm font-semibold">Jabatan dapat melakukan verifikasi</span><span class="block text-xs text-slate-500">Pemegang aktif jabatan ini dapat dipilih sebagai verifikator.</span></span></label>
                </div>
    ...
    @if($showHolderForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" wire:click.self="$set('showHolderForm', false)">
            <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
                <h2 class="text-lg font-semibold">Tetapkan Pemegang Jabatan</h2><p class="mt-1 text-sm text-slate-500">Data pejabat aktif dimuat otomatis. Saat diganti, riwayat pejabat sebelumnya tetap tersimpan.</p>
                <div class="mt-5 space-y-4">
                    <div><label class="text-sm font-medium text-slate-700">Pejabat</label><select wire:model="holderUserId" class="form-select mt-1"><option value="">Pilih user...</option>@foreach($users as $user)<option value="{{ $user->id }}">{{ $user->name }} — {{ $user->email }}</option>@endforeach</select>@error('holderUserId')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                    <div><label class="text-sm font-medium text-slate-700">Tanggal Mulai</label><input wire:model="holderStartedAt" type="date" class="form-control mt-1">@error('holderStartedAt')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                </div>
