        ...
        <div class="overflow-x-auto"><table class="min-w-full divide-y divide-slate-200 text-sm"><thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500"><tr><th class="px-5 py-3">Jabatan</th>@if($isSuperAdmin)<th class="px-5 py-3">Organisasi</th>@endif<th class="px-5 py-3">Pejabat Aktif</th><th class="px-5 py-3">Penandatangan</th><th class="px-5 py-3">Verifikator</th><th class="px-5 py-3">Sertifikat TTE</th><th class="px-5 py-3">Status</th><th class="px-5 py-3 text-right">Aksi</th></tr></thead>
            <tbody class="divide-y divide-slate-100">
                @forelse ($positions as $position)
                    ...
                    <tr>
                        <td class="px-5 py-4"><div class="font-semibold text-slate-900">{{ $position->name }}</div><div class="text-xs text-slate-500">{{ $position->code }}</div></td>@if($isSuperAdmin)<td class="px-5 py-4 text-sm text-slate-700">{{ $position->tenant?->name ?? '—' }}</td>@endif
                ...
                @empty<tr><td colspan="{{ $isSuperAdmin ? 8 : 7 }}" class="px-5 py-12 text-center text-sm text-slate-500">Belum ada jabatan.</td></tr>@endforelse
            </tbody></table></div>
