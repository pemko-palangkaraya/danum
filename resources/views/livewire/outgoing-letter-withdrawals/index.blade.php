<div class="space-y-6">
                ...
                @forelse($pendingRequests as $request)
                    <div class="p-5">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2"><span class="font-mono text-xs font-semibold text-slate-400">{{ $request->outgoingLetter->number }}</span><span class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">Pending</span></div>
                                ...
                                <div class="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900"><span class="font-semibold">Alasan penarikan:</span> {{ $request->reason }}</div>
                            ...
                            <div class="flex shrink-0 gap-2"><a href="{{ route('outgoing-letters.show', $request->outgoingLetter->id) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Lihat Surat</a><button wire:click="openDecision('{{ $request->id }}')" class="rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">Proses</button></div>
                        </div>
                    </div>
                @empty
    ...
    @else
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4"><h2 class="font-semibold text-slate-900">Surat Terbit Milik Saya</h2><p class="mt-1 text-xs text-slate-500">Penarikan hanya dapat diajukan oleh pembuat surat.</p></div>
            <div class="divide-y divide-slate-100">
                @forelse($issuedLetters as $letter)
                    @php($pending = $letter->withdrawalRequests->first(fn($r) => $r->status->value === 'pending'))
                    <div class="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between"><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><span class="font-mono text-xs font-semibold text-slate-400">{{ $letter->number }}</span><span class="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700">Issued</span></div><h3 class="mt-1 font-semibold text-slate-900">{{ $letter->subject }}</h3><p class="mt-1 text-sm text-slate-500">{{ $letter->recipient_name }} · {{ $letter->letterType?->name }}</p></div><div class="flex shrink-0 gap-2"><a href="{{ route('outgoing-letters.show', $letter->id) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Lihat Surat</a>@if($pending)<span class="rounded-lg bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700">Menunggu Persetujuan</span>@else<button wire:click="openRequest('{{ $letter->id }}')" class="rounded-lg bg-red-700 px-3 py-2 text-sm font-semibold text-white hover:bg-red-800">Ajukan Penarikan</button>@endif</div></div>
                @empty
