        ...
        <div class="relative z-10 flex max-h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div class="overflow-y-auto px-3 py-4 sm:px-6 sm:py-5">
                ...
                <div class="w-full overflow-x-auto pb-1">
                    <div class="mx-auto w-full min-w-0 max-w-[680px] sm:max-w-5xl sm:min-w-[620px]">
                        <div class="mb-3 grid grid-cols-[minmax(0,1fr)_44px_minmax(0,1fr)] items-center gap-1 text-[10px] font-semibold uppercase tracking-wide sm:grid-cols-[1fr_64px_1fr] sm:gap-2 sm:text-xs"><div class="text-right text-sky-600">Laki-laki</div><div></div><div class="text-pink-600">Perempuan</div></div>
                        <div class="space-y-1">
                            @foreach($ageGroups as $label => $group)
                                <div class="grid grid-cols-[minmax(0,1fr)_44px_minmax(0,1fr)] items-center gap-1 sm:grid-cols-[1fr_64px_1fr] sm:gap-2"><div class="flex min-w-0 items-center justify-end gap-1 sm:gap-2"><span class="w-7 shrink-0 text-right text-[9px] font-medium text-slate-400 sm:w-12 sm:text-[11px]">{{ number_format($group['male']) }}</span><div class="flex h-4 min-w-0 w-full justify-end overflow-hidden rounded-l bg-sky-100 sm:h-5"><div class="h-full rounded-l bg-sky-500 transition-all" style="width: {{ $group['male_width'] }}%"></div></div></div><div class="text-center text-[10px] font-semibold text-slate-600 sm:text-[11px]">{{ $label }}</div><div class="flex min-w-0 items-center gap-1 sm:gap-2"><div class="h-4 min-w-0 w-full overflow-hidden rounded-r bg-pink-100 sm:h-5"><div class="h-full rounded-r bg-pink-500 transition-all" style="width: {{ $group['female_width'] }}%"></div></div><span class="w-7 shrink-0 text-[9px] font-medium text-slate-400 sm:w-12 sm:text-[11px]">{{ number_format($group['female']) }}</span></div></div>
                            @endforeach
                        </div>
