        ...
        @if($memberSearch !== '')
            <div class="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
                @forelse($memberCandidates as $citizen)
                    @php($canAddCitizen = $memberRelationship !== 'spouse' || $citizen->jenis_kelamin === 'female')
                    <div class="flex items-center justify-between gap-4 border-b border-slate-100 px-4 py-3 last:border-0 hover:bg-slate-50">
                        ...
                        <button
                            type="button"
                            wire:click="addMember('{{ $detail->id }}', '{{ $citizen->id }}', '{{ $memberRelationship }}')"
                            @disabled(! $canAddCitizen)
                            class="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            ...
                            {{ $canAddCitizen ? 'Tambahkan' : 'Bukan Istri' }}
                        </button>
                    </div>
                @empty
