<aside class="hidden w-64 shrink-0 border-r border-slate-200 bg-white lg:flex lg:flex-col">
    ...
    <nav class="flex-1 space-y-1 px-4 py-6">
        ...
        @if (auth()->user()?->isSuperAdmin())
            ...
            <a href="{{ route('positions.index') }}" @class(['flex items-center rounded-xl px-3 py-2.5 text-sm font-medium transition','bg-slate-900 text-white shadow-sm'=>request()->routeIs('positions.*'),'text-slate-600 hover:bg-slate-100 hover:text-slate-900'=>!request()->routeIs('positions.*')])>Jabatan</a>
        @endif

        @if (auth()->user()?->isTenantUser())
            <a href="{{ route('tenant.positions.index') }}" @class(['flex items-center rounded-xl px-3 py-2.5 text-sm font-medium transition','bg-slate-900 text-white shadow-sm'=>request()->routeIs('tenant.positions.*'),'text-slate-600 hover:bg-slate-100 hover:text-slate-900'=>!request()->routeIs('tenant.positions.*')])>Jabatan</a>
