    ...
    <div class="border-t border-slate-100 p-4">
        @if (auth()->user()?->isTenantUser() && auth()->user()?->hasPermission('outgoing-letters.issue'))
            ...
            <a
                ...
                @class([
                    'mb-3 flex items-center rounded-xl px-3 py-2.5 text-sm font-medium transition',
                    'bg-slate-100 text-slate-900' => request()->routeIs('settings.signing-pin'),
                    'text-slate-600 hover:bg-slate-100 hover:text-slate-900' => !request()->routeIs('settings.signing-pin'),
                ])>
                <svg class="mr-3 h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.5 8.5a4 4 0 1 0-5.66 5.66L13 17.33V21h3v-2h2v-2h2v-3.67l-4.5-4.5Z" />
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 11.5h.01" />
                </svg>
                PIN Tanda Tangan
            </a>
        @endif
