...
class OutgoingLetterController extends Controller
{
    ...
    public function store(StoreOutgoingLetterRequest $request): JsonResponse
    {
        ...
        if (! empty($data['validator_position_id'])) {
            ...
            if (! $validator || ! $holder?->user) return response()->json(['message' => 'Validator position is unavailable or has no active holder.', 'errors' => ['validator_position_id' => ['The selected validator position is not currently available.']]], 422);
    ...
    public function update(UpdateOutgoingLetterRequest $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            if (array_key_exists('signer_position_id', $data)) {
                $signer = $this->availablePosition($request->user()->tenant_id, $data['signer_position_id'], 'can_sign'); $holder = $signer?->holders->first();
                if (! $signer || ! $holder?->user) return response()->json(['message' => 'Signer position is unavailable or has no active holder.'], 422);
                $data['signer_user_id'] = $holder->user_id; $data['signer_name'] = $holder->user->name; $data['signer_title'] = $signer->name;
            }
            if (array_key_exists('validator_position_id', $data)) {
                $validator = $this->availablePosition($request->user()->tenant_id, $data['validator_position_id'], 'can_validate'); $holder = $validator?->holders->first();
                if (! $validator || ! $holder?->user) return response()->json(['message' => 'Validator position is unavailable or has no active holder.'], 422);
                $data['validator_user_id'] = $holder->user_id; $data['validator_name'] = $holder->user->name; $data['validator_title'] = $validator->name;
            }
    ...
    {
        $outgoingLetter = $this->findForTenant($id, $request); if ($outgoingLetter === null) abort(404, 'Outgoing letter not found.'); $this->authorize('view', $outgoingLetter);
        $outgoingLetter->loadMissing(['tenant', 'letterType', 'letterTypeVersion', 'signerPosition', 'signerUser', 'validatorPosition', 'validatorUser']);
        $sourceDocxPath = $outgoingLetter->generated_docx_path; if (! $sourceDocxPath || ! Storage::disk('local')->exists($sourceDocxPath)) abort(422, 'DOCX hasil surat belum tersedia. Buat ulang draft surat terlebih dahulu.');
        $isIssued = $outgoingLetter->status === OutgoingLetterStatus::ISSUED; $temporaryDocx = null;
        try {
            if ($isIssued) { if (blank($outgoingLetter->verification_token)) abort(422, 'Surat terbit belum memiliki token verifikasi. Terbitkan ulang surat tersebut.'); $temporaryDocx = $this->docxTteService->createIssuedCopy(Storage::disk('local')->path($sourceDocxPath), url('/verify/' . $outgoingLetter->verification_token)); $pdfPath = $this->docxPdfService->convert($temporaryDocx); }
            else { $temporaryDocx = $this->docxTteService->createPreviewCopy(Storage::disk('local')->path($sourceDocxPath)); $pdfPath = $this->docxPdfService->convert($temporaryDocx); }
            $absolutePath = Storage::disk('local')->path($pdfPath);
            if (! $isIssued) { $label = sprintf('%s | %s | PREVIEW', $request->user()->name, now()->format('d-m-Y')); $watermarked = $this->pdfPreviewWatermarkService->apply($absolutePath, $label); $filename = sprintf('preview-surat-%s.pdf', str($outgoingLetter->number)->slug()); return response()->file($watermarked, ['Content-Type' => 'application/pdf', 'Content-Disposition' => 'inline; filename="' . $filename . '"'])->deleteFileAfterSend(true); }
        } catch (\RuntimeException $exception) { abort(422, $exception->getMessage()); } finally { if ($temporaryDocx !== null) @unlink($temporaryDocx); }
