...
class PopulationExportController extends Controller
{
    ...
    private function citizenRow(Citizen $citizen, ?PopulationReferenceService $references): array
    {
        return [
            ...
            $references?->label('gender', $citizen->jenis_kelamin, (string) $citizen->jenis_kelamin) ?? $citizen->jenis_kelamin,
            $references?->label('blood_type', $citizen->golongan_darah, (string) $citizen->golongan_darah) ?? $citizen->golongan_darah,
            $references?->label('religion', $citizen->agama, (string) $citizen->agama) ?? $citizen->agama,
            $references?->label('marital_status', $citizen->status_perkawinan, (string) $citizen->status_perkawinan) ?? $citizen->status_perkawinan,
            $citizen->pendidikan,
            $citizen->pekerjaan,
