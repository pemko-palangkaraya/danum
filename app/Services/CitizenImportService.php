...
class CitizenImportService
{
    ...
    public function tenantExists(string $tenantId): bool { return Str::isUuid($tenantId) && Tenant::query()->whereKey($tenantId)->exists(); }
    public function tenants(): Collection { return Tenant::query()->orderBy('name')->get(['id', 'name', 'code']); }
    ...
    public function preview(UploadedFile $file, string $tenantId, string $duplicateMode): array
    {
        ...
        foreach ($rows as $i => $item) {
            ...
            if ($existingNiks->has('nik:'.$item['nik'])) {
    ...
    private function validateTenant(string $tenantId): void { if (! $this->tenantExists($tenantId)) throw ValidationException::withMessages(['tenant_id' => 'Tenant tidak valid atau tidak ditemukan.']); }
    private function validateDuplicateMode(string $duplicateMode): void { Validator::make(['duplicate_mode' => $duplicateMode], ['duplicate_mode' => ['required', 'string', Rule::in(self::DUPLICATE_MODES)]])->validate(); }
    private function rowRules(): array { return ['nik' => ['required', 'digits:16'], 'nama_lengkap' => ['required', 'string', 'max:255'], 'tanggal_lahir' => ['nullable', 'date'], 'jenis_kelamin' => ['nullable', 'in:male,female'], 'golongan_darah' => ['nullable', 'in:A,B,AB,O,unknown'], 'nik_ayah' => ['nullable', 'digits:16'], 'nik_ibu' => ['nullable', 'digits:16']]; }
    ...
    {
        $handle = fopen($path, 'rb'); if ($handle === false) throw new RuntimeException('File import tidak dapat dibaca.');
        try { $rows = []; while (($row = fgetcsv($handle, 0, ';')) !== false) { if (count($row) === 1 && str_contains((string) $row[0], ',')) $row = str_getcsv($row[0], ','); $rows[] = $row; } return $rows; }
        finally { fclose($handle); }
    }
    private function headerMap(array $headers): array { $map = []; foreach ($headers as $index => $header) { $header = preg_replace('/^\xEF\xBB\xBF/', '', (string) $header); $map[strtolower(trim($header))] = $index; } return $map; }
