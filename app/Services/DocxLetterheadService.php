...
class DocxLetterheadService
{
    ...
    public function embed(string $xml, string $rels, string $contentTypes, Tenant $tenant): ?array
    {
        ...
        if ($paragraphs) {
            foreach ($paragraphs as $paragraph) {
                if (str_contains($this->nodeText($xpath, $paragraph), '{{letterhead}}')) {
                    $targets[] = $paragraph;
                }
            }
        }
        ...
        if (! $structured) {
            return $this->embedLegacyImage($dom, $xpath, $targets, $rels, $contentTypes, $tenant);
        }
        ...
        if ($logoPath !== null) {
            ...
            if (! in_array($extension, ['png', 'jpg', 'jpeg'], true)) {
                throw new RuntimeException('Logo kop surat harus berupa PNG, JPG, atau JPEG.');
            }

            $mediaName = 'danum-letterhead-logo-' . substr(sha1($logoPath), 0, 12) . '.' . $extension;
            $rid = 'rIdDanumLetterheadLogo';
            $relsDom = new DOMDocument();
            $relsDom->preserveWhiteSpace = true;
            if (! $relsDom->loadXML($rels, LIBXML_NOBLANKS | LIBXML_NOERROR | LIBXML_NOWARNING)) {
                throw new RuntimeException('DOCX relationships tidak valid.');
            }
    ...
    {
        if (! $tenant->logo || str_starts_with($tenant->logo, 'http://') || str_starts_with($tenant->logo, 'https://') || str_starts_with($tenant->logo, '/')) {
            return null;
        }
    ...
    {
        return collect([
            $tenant->letterhead_line1,
            $tenant->letterhead_line2,
            $tenant->letterhead_line3,
            $tenant->postal_code,
            $tenant->address,
            $tenant->website,
            $tenant->email,
    ...
    private function embedLegacyImage(DOMDocument $dom, DOMXPath $xpath, array $targets, string $rels, string $contentTypes, Tenant $tenant): ?array
    {
        ...
        if (! in_array($extension, ['png', 'jpg', 'jpeg', 'gif'], true)) {
            throw new RuntimeException('Kop surat harus berupa PNG, JPG, JPEG, atau GIF.');
        }
        ...
        foreach ($targets as $paragraph) {
            ...
            foreach ($nodes as $node) {
                if (! str_contains($node->textContent, '{{letterhead}}')) continue;
    ...
    private function buildTableXml(Tenant $tenant, string $mediaName, bool $hasLogo): string
    {
        $rows = [];
        foreach ([
            [$tenant->letterhead_line1, '30', true],
            [$tenant->letterhead_line2, '26', true],
            [$tenant->letterhead_line3, '22', true],
        ] as [$value, $size, $bold]) {
            if (trim((string) $value) === '') continue;
            $rows[] = $this->textParagraph((string) $value, (int) $size, $bold);
        }
        ...
        if ($meta !== []) $rows[] = $this->textParagraph(implode('  |  ', $meta), 17, false);
        if ($rows === []) $rows[] = $this->textParagraph($tenant->name, 30, true);

        $textCell = '<w:tc><w:tcPr><w:tcW w:w="' . ($hasLogo ? '7600' : '9000') . '" w:type="dxa"/><w:vAlign w:val="center"/></w:tcPr><w:p><w:pPr><w:jc w:val="center"/><w:spacing w:before="0" w:after="0"/></w:pPr></w:p>' . implode('', $rows) . '</w:tc>';
        $logoCell = '';
        if ($hasLogo) {
            $logoCell = '<w:tc><w:tcPr><w:tcW w:w="1400" w:type="dxa"/><w:vAlign w:val="center"/></w:tcPr><w:p><w:pPr><w:jc w:val="center"/><w:spacing w:before="0" w:after="0"/></w:pPr><w:r><w:drawing><wp:inline xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture" xmlns:r="' . self::OFFICE_REL_NS . '" distT="0" distB="0" distL="0" distR="0"><wp:extent cx="900000" cy="900000"/><wp:docPr id="9002" name="DANUM Letterhead Logo"/><a:graphic><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic><pic:nvPicPr><pic:cNvPr id="0" name="' . htmlspecialchars($mediaName, ENT_XML1 | ENT_QUOTES, 'UTF-8') . '"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="rIdDanumLetterheadLogo"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="900000" cy="900000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p></w:tc>';
        }

        return '<w:tbl xmlns:w="' . DocxRendererService::WORD_NS . '"><w:tblPr><w:tblW w:w="9000" w:type="dxa"/><w:tblLayout w:type="fixed"/><w:tblBorders><w:top w:val="nil"/><w:left w:val="nil"/><w:bottom w:val="single" w:sz="16" w:space="1"/><w:right w:val="nil"/><w:insideH w:val="nil"/><w:insideV w:val="nil"/></w:tblBorders><w:tblCellMar><w:top w:w="0" w:type="dxa"/><w:left w:w="0" w:type="dxa"/><w:bottom w:w="90" w:type="dxa"/><w:right w:w="0" w:type="dxa"/></w:tblCellMar></w:tblPr><w:tr>' . $logoCell . $textCell . '</w:tr></w:tbl>';
    }

    private function textParagraph(string $text, int $halfPoints, bool $bold): string
    {
