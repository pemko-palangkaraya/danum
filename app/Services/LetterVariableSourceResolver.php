...
final class LetterVariableSourceResolver
{
    ...
    public function citizen(Citizen $citizen): array
    {
        return [
            ...
            'citizen_jenis_kelamin' => $this->gender($citizen->jenis_kelamin),
            ...
            'recipient_gender' => $this->gender($citizen->jenis_kelamin),
    ...
    /** @return array{values:array<string,string>, children:list<array{nomor:string,nama:string}>} */
    public function family(Citizen $citizen): array
    {
        $family = $this->familyFor($citizen);
        if (! $family) {
            return ['values' => ['recipient_address' => '', 'nama_pasangan' => '-'], 'children' => [['nomor' => '1', 'nama' => '-']]];
        }
        ...
        $children = $members
            ->filter(fn ($member) => mb_strtolower(trim((string) $member->hubungan_dalam_keluarga)) === 'anak')
            ->values()
