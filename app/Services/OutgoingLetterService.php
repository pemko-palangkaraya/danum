...
{
    public function __construct(private readonly OutgoingLetterRepositoryInterface $repository, private readonly OutgoingLetterStatusHistoryRepositoryInterface $historyRepository, private readonly LetterTypeService $letterTypeService) {}
    ...
    public function create(array $data, int $changedBy): OutgoingLetter
    {
        ...
        if(!$letterType) throw new \DomainException('Jenis surat tidak ditemukan.'); if($letterType->status->value!=='active') throw new \DomainException('Jenis surat tidak aktif.');
        ...
        $letter=$this->repository->create($data); $this->recordHistory($letter,'created',$changedBy); return $letter;
    }
    ...
    public function submit(OutgoingLetter $letter,int $changedBy): OutgoingLetter { if($letter->status!==OutgoingLetterStatus::DRAFT) throw new \DomainException('Hanya draft yang dapat dikirim untuk verifikasi.'); if($letter->submitted_at!==null) throw new \DomainException('Surat sudah dikirim untuk verifikasi.'); if($letter->validator_user_id===null) throw new \DomainException('Verifikator belum ditentukan.'); $letter=$this->repository->update($letter,['submitted_at'=>now(),'rejection_reason'=>null,'rejected_by'=>null,'rejected_at'=>null]); $this->recordHistory($letter,'submitted',$changedBy); return $letter; }
    ...
    public function issue(OutgoingLetter $letter,int $changedBy): OutgoingLetter { $letterType=$letter->letterType()->first(); $issuedAt=now(); $attributes=['issued_at'=>$issuedAt->toDateString(),'valid_from'=>$issuedAt,'valid_until'=>null]; $period=$letterType?->validity_period??'none'; if($period!=='none') $attributes['valid_until']=match($period){'1_week'=>$issuedAt->copy()->addWeek(),'2_weeks'=>$issuedAt->copy()->addWeeks(2),'1_month'=>$issuedAt->copy()->addMonth(),'3_months'=>$issuedAt->copy()->addMonths(3),'6_months'=>$issuedAt->copy()->addMonths(6),'1_year'=>$issuedAt->copy()->addYear(),default=>throw new \DomainException('Masa berlaku jenis surat tidak valid.')}; return $this->transition($letter,OutgoingLetterStatus::VALIDATED,OutgoingLetterStatus::ISSUED,$changedBy,$attributes); }
