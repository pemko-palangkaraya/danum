...
class DocxTteService
{
    ...
    /** Create a temporary copy containing the official QR/TTE. The source DOCX is never modified. */
    public function createIssuedCopy(string $sourcePath, string $verificationUrl): string
    {
        ...
        try {
            $this->embed($copy, $verificationUrl);
        } catch (\Throwable $e) {
            @unlink($copy);
            throw $e;
        }
    ...
    /** Create a preview copy and remove any stale TTE QR left by older builds. */
    ...
    private function temporaryCopy(string $sourcePath, string $prefix): string
    {
        ...
        if ($tmp === false || ! copy($sourcePath, $tmp)) {
            ...
            throw new RuntimeException('Tidak dapat membuat salinan DOCX sementara.');
        }
        return $tmp;
    }
    ...
    private function removeTte(string $docxPath): void
    {
        ...
        if ($drawings) {
            foreach ($drawings as $drawing) $drawing->parentNode?->removeChild($drawing);
        }
        $zip->addFromString('word/document.xml', $dom->saveXML() ?: $xml);
        $zip->close();
    }
