...
{
    /** @return array{key:string,label:string,source:?string,fields:list<array{key:string,label:string}>} */
    public static function parseRepeater(string $definition): ?array
    {
        if (! preg_match('/^@repeat\s+([A-Za-z_][A-Za-z0-9_]*)(?::([A-Za-z_][A-Za-z0-9_]*))?\s*\|\s*(.+)$/', trim($definition), $matches)) {
            return null;
        }

        $fields = [];
        foreach (preg_split('/\s*,\s*/', trim($matches[3])) ?: [] as $field) {
            $parts = array_map('trim', explode(':', $field, 2));
            $key = $parts[1] ?? $parts[0];
            $label = $parts[0];

            if (! preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $key)) return null;
    ...
    /** @param list<string> $variables @return list<array{key:string,label:string,source:?string,fields:list<array{key:string,label:string}>}> */
