...
final class LetterVariableSchema
{
    ...
    {
        if (! preg_match('/^@repeat\s+([A-Za-z_][A-Za-z0-9_]*)\s*\|\s*(.+)$/', trim($definition), $matches)) return null;
        $fields = [];
    ...
    public static function isRepeater(string $definition): bool { return self::parseRepeater($definition) !== null; }
    ...
    {
        $result = [];
        foreach ($variables as $variable) if (is_string($variable) && ($parsed = self::parseRepeater($variable))) $result[] = $parsed;
        return $result;
    }
}
