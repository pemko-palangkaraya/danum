...
class Index extends Component
{
    ...
    {
        try {
            $letter = $this->tenantQuery()->findOrFail($id); $this->authorize('update', $letter); $letterType = $letter->letterType; $tenantId = auth()->user()->tenant_id;
            ...
            $version = $letter->letterType ? app(LetterTypeService::class)->activeVersion($letter->letterType) : null; $this->variables = $version?->variables ?? $letter->letterType?->variables ?? []; $this->variableValues = $letter->input_data ?? [];
            foreach ($this->variables as $variable) { if (LetterVariableSchema::isRepeater((string) $variable)) { $definition = LetterVariableSchema::parseRepeater((string) $variable); $this->variableValues[$definition['key']] ??= []; } else $this->variableValues[$variable] ??= ''; }
    ...
    public function updatedLetterTypeId(): void
    {
        ...
        $version = app(LetterTypeService::class)->activeVersion($type); $this->variables = $version?->variables ?? $type->variables ?? []; $this->variableValues = [];
        foreach ($this->variables as $variable) { if (LetterVariableSchema::isRepeater((string) $variable)) { $definition = LetterVariableSchema::parseRepeater((string) $variable); $this->variableValues[$definition['key']] = [[]]; } else $this->variableValues[$variable] = ''; }
    ...
    public function addRepeaterRow(string $key): void
    {
        ...
        $row = []; foreach ($definition['fields'] as $field) $row[$field['key']] = '';
        $this->variableValues[$key] ??= []; $this->variableValues[$key][] = $row;
    }

    public function removeRepeaterRow(string $key, int $index): void
    {
        if (! isset($this->variableValues[$key][$index])) return;
        unset($this->variableValues[$key][$index]); $this->variableValues[$key] = array_values($this->variableValues[$key]);
    ...
    {
        try {
            $tenantId = auth()->user()->tenant_id; if ($tenantId === null) throw new \DomainException('Akun platform tidak dapat membuat surat tanpa konteks tenant.');
            ...
            $letterType = app(LetterTypeService::class)->getAvailableForTenant($tenantId)->firstWhere('id', $this->letter_type_id); if (! $letterType) throw new \DomainException('Jenis surat tidak tersedia atau belum diberikan akses ke OPD Anda.');
            $position = $this->availableSignerPositions()->find($this->signer_position_id); $holder = $position?->holders->first(); if (! $position || ! $holder?->user) throw new \DomainException('Jabatan penanda tangan tidak tersedia atau belum memiliki pejabat aktif.');
            $validatorPosition = $this->availableValidatorPositions()->find($this->validator_position_id); $validatorHolder = $validatorPosition?->holders->first(); if (! $validatorPosition || ! $validatorHolder?->user) throw new \DomainException('Jabatan verifikator tidak tersedia atau belum memiliki pejabat aktif.');
            ...
            $letterTypeVersion = app(LetterTypeService::class)->activeVersion($letterType); $templateRelativePath = $letterTypeVersion?->template_path ?: $letterType->template_path; if (! $templateRelativePath) throw new \DomainException('Template DOCX surat belum tersedia.'); $templatePath = Storage::disk('local')->path($templateRelativePath); if (! is_file($templatePath)) throw new \DomainException('File template DOCX tidak ditemukan di storage.');
    ...
    private function availablePositions(string $capability) { $tenant = auth()->user()?->tenant; if (! $tenant) return Position::query()->whereRaw('1 = 0'); return Position::query()->where('tenant_category_id', $tenant->tenant_category_id)->where('status', PositionStatus::ACTIVE)->where($capability, true)->whereHas('holders', fn ($query) => $query->where('tenant_id', $tenant->id)->whereNull('ended_at')->where('started_at', '<=', now()))->with(['holders' => fn ($query) => $query->where('tenant_id', $tenant->id)->whereNull('ended_at')->where('started_at', '<=', now())->with('user')]); }
    ...
    {
        if ($this->filter === null || $this->filter === '') $this->filter = 'all';
        ...
        return view('livewire.pages.outgoing-letters.index', ['letters' => $letters->paginate($this->perPage), 'letterTypes' => $letterTypes, 'signerPositions' => $this->availableSignerPositions()->orderBy('name')->get(), 'validatorPositions' => $this->availableValidatorPositions()->orderBy('name')->get(), 'variableLabels' => (new DocxTemplateService)->allowedVariables(), 'repeaters' => $this->repeaterDefinitions(), 'isSuperAdmin' => $this->isSuperAdmin()]);
    }
}
