...
class Index extends Component
{
    ...
    public function updatedLetterTypeId(): void
    {
        ...
        $this->variables = $type?->variables ?? ['number', 'recipient_name', 'recipient_address', 'subject'];
        $this->variableValues = array_fill_keys($this->variables, '');
    }

    public function save(OutgoingLetterService $service, LetterTypeService $letterTypeService, OutgoingLetterTemplateService $templateService): void
    {
        $data = $this->validate([
        ...
        $this->recipient_name = (string) ($data['recipient_name'] ?? '');
        $this->recipient_address = (string) ($data['recipient_address'] ?? '');
        $this->subject = (string) ($data['subject'] ?? '');

        $version = $letterTypeService->ensureCurrentVersion($letterType);
        $content = $version ? $templateService->renderVersion($version, auth()->user()->tenant, $data) : '';
        if ($content === '') { $this->addError('letter_type_id', 'Template surat belum tersedia.'); return; }

        $service->create([...$data, 'tenant_id' => auth()->user()->tenant_id, 'letter_type_version_id' => $version?->id, 'content' => $content, 'status' => OutgoingLetterStatus::DRAFT], auth()->id());
        $this->showForm = false; $this->resetForm(); $this->dispatch('toast', type: 'success', message: 'Draft surat berhasil dibuat.');
    }
    ...
    private function resetForm(): void { $this->reset(['letter_type_id','variables','variableValues','number','recipient_name','recipient_address','subject']); }
    ...
    public function render()
    {
        ...
        return view('livewire.pages.outgoing-letters.index', ['letters'=>$letters->paginate($this->perPage),'letterTypes'=>LetterType::query()->whereNull('tenant_id')->where('status',LetterTypeStatus::ACTIVE)->orderBy('name')->get(),'variableLabels'=>(new OutgoingLetterTemplateService)->variables()]);
    }
}
