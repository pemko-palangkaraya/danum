...
class Index extends Component
{
    ...
    public function save(OutgoingLetterService $service, DocxTemplateService $docx, DocxTteService $tte): void
    {
        try {
            ...
            if ($existing) {
                ...
                $service->create($attributes, auth()->id());
    ...
    public function issue(string $id, OutgoingLetterService $service, ?string $note = null, ?string $pin = null): void
    {
        ...
        try {
            ...
            $service->issue($letter, auth()->id(), $note, $pin);
            $this->dispatch('toast', type: 'success', message: 'Surat berhasil diterbitkan.');
        } catch (\Throwable $exception) {
            $this->toastError($exception instanceof \DomainException ? $exception->getMessage() : 'Surat gagal diterbitkan. Anda mungkin tidak memiliki kewenangan atau status surat sudah berubah.');
        }
    }
    ...
    public function handleWorkflowNote(string $action, string $id, string $note, OutgoingLetterService $service): void
    {
        $note = trim($note);
        if ($note === '') {
            $this->dispatch('workflow-note-required', action: $action, id: $id, title: $action === 'validate' ? 'Catatan Verifikasi' : 'Catatan Penandatanganan', description: $action === 'validate' ? 'Catatan wajib diisi.' : 'Catatan wajib diisi.');
            return;
        }
    ...
    {
        return Position::query()->where('tenant_id', auth()->user()->tenant_id)->where('status', PositionStatus::ACTIVE)->where($capability, true)->whereHas('holders', fn($query) => $query->whereNull('ended_at')->where('started_at', '<=', now()))->with(['holders' => fn($query) => $query->whereNull('ended_at')->where('started_at', '<=', now())->with('user')]);
    }
    ...
    {
        return OutgoingLetter::query()->where('tenant_id', auth()->user()->tenant_id)->with(['letterType']);
    }
