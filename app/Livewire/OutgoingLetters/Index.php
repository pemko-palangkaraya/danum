...
class Index extends Component
{
    ...
    public function create(): void { try { $this->authorize('create', OutgoingLetter::class); $this->resetForm(); $this->showForm = true; } catch (\Throwable) { $this->toastError('Anda tidak memiliki izin untuk membuat surat.'); } }

    public function edit(string $id): void
    {
        try {
            $letter = $this->tenantQuery()->findOrFail($id); $this->authorize('update', $letter); $letterType = $letter->letterType;
            if (! $letterType || ! app(LetterTypeService::class)->isAllowedForTenant($letterType, (string) auth()->user()->tenant_id)) throw new \DomainException('Jenis surat ini sudah tidak diberikan akses ke OPD Anda.');
            $this->editingId = $letter->id; $this->letter_type_id = $letter->letter_type_id; $this->signer_position_id = $letter->signer_position_id; $this->validator_position_id = $letter->validator_position_id; $this->variables = $letter->letterType?->variables ?? []; $this->variableValues = $letter->input_data ?? [];
            foreach ($this->variables as $variable) $this->variableValues[$variable] ??= '';
            $this->variableValues['number'] = $letter->number; $this->variableValues['recipient_name'] = $letter->recipient_name; $this->variableValues['recipient_address'] = $letter->recipient_address; $this->variableValues['subject'] = $letter->subject; $this->variableValues['date'] = optional($letter->letter_date)->format('Y-m-d'); $this->applySystemValues(); $this->resetValidation(); $this->showForm = true;
    ...
    {
        $type = app(LetterTypeService::class)->getAvailableForTenant((string) auth()->user()->tenant_id)->firstWhere('id', $this->letter_type_id);
        ...
        $this->variables = $type->variables ?? []; $this->variableValues = array_fill_keys($this->variables, ''); $this->applySystemValues();
    }
    ...
    public function save(OutgoingLetterService $service, DocxTemplateService $docx, DocxTteService $tte): void
    {
        try {
            ...
            $letterType = app(LetterTypeService::class)->getAvailableForTenant((string) auth()->user()->tenant_id)->firstWhere('id', $this->letter_type_id); if (! $letterType) throw new \DomainException('Jenis surat tidak tersedia atau belum diberikan akses ke OPD Anda.');
            ...
            if (! $letterType->template_path) throw new \DomainException('Template DOCX surat belum tersedia.'); $templatePath = Storage::disk('local')->path($letterType->template_path); if (! is_file($templatePath)) throw new \DomainException('File template DOCX tidak ditemukan di storage.');
            $existing = $this->editingId ? $this->tenantQuery()->findOrFail($this->editingId) : null; if ($existing) $this->authorize('update', $existing); $verificationToken = $existing?->verification_token ?? Str::random(64); $generatedPath = $docx->renderToStorage($templatePath, auth()->user()->tenant, $data); $tte->embed(Storage::disk('local')->path($generatedPath), url('/verify/' . $verificationToken)); $content = $docx->extractText(Storage::disk('local')->path($generatedPath));
    ...
    {
        try { $letter = $this->tenantQuery()->findOrFail($id); $this->authorize('submit', $letter); $service->submit($letter, auth()->id()); $this->dispatch('toast', type: 'success', message: 'Surat dikirim untuk verifikasi. Data sekarang terkunci.'); } catch (\Throwable $exception) { $this->toastError($exception instanceof \DomainException ? $exception->getMessage() : 'Surat gagal dikirim untuk verifikasi.'); }
    }

    public function openReject(string $id): void
    {
        try { $letter = $this->tenantQuery()->findOrFail($id); $this->authorize('reject', $letter); $this->rejectId = $id; $this->rejectReason = ''; $this->resetValidation(); $this->showRejectForm = true; } catch (\Throwable) { $this->toastError('Anda tidak memiliki kewenangan untuk menolak surat ini.'); }
    }
    ...
    private function availablePositions(string $capability) { return Position::query()->where('tenant_id', auth()->user()->tenant_id)->where('status', PositionStatus::ACTIVE)->where($capability, true)->whereHas('holders', fn ($query) => $query->whereNull('ended_at')->where('started_at','<=',now()))->with(['holders' => fn ($query) => $query->whereNull('ended_at')->where('started_at','<=',now())->with('user')]); }
    ...
    public function render()
    {
        ...
        $letterTypeService = app(LetterTypeService::class); $letterTypes = $this->isSuperAdmin() ? collect() : $letterTypeService->getAvailableForTenant((string) auth()->user()->tenant_id)->sortBy('name')->values();
