...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if ($tenantCategoryId === null) throw new \RuntimeException('Master tenant category "lainnya" tidak ditemukan.');
        ...
        $tenant = Tenant::firstOrCreate(['code' => env('DANUM_DEMO_TENANT_CODE', 'DEMO001')], [
            ...
            'province' => 'Kalimantan Tengah', 'city' => 'Palangka Raya', 'district' => 'Demo', 'village' => 'Demo',
            'address' => 'Demo', 'phone' => null, 'email' => 'demo@example.com', 'logo' => null,
            'head_name' => 'Demo Head', 'head_title' => 'Kepala Unit', 'status' => TenantStatus::ACTIVE,
        ]);

        $tenantAdminRole = Role::query()->updateOrCreate(['tenant_id' => null, 'slug' => 'tenant_admin'], [
            'name' => 'Tenant Admin', 'scope' => 'tenant', 'is_system' => true, 'is_active' => true,
        ]);

        User::updateOrCreate(['email' => env('DANUM_TENANT_ADMIN_EMAIL', 'yudhistira@danum.local')], [
            'name' => 'Tenant Admin', 'nip' => null, 'email_verified_at' => now(),
            'password' => Hash::make(env('DANUM_TENANT_ADMIN_PASSWORD', 'password')), 'remember_token' => null,
            'platform_role' => null, 'custom_role_id' => $tenantAdminRole->id, 'status' => UserStatus::ACTIVE, 'tenant_id' => $tenant->id,
        ]);

        $tenantUserRole = Role::query()->updateOrCreate(['tenant_id' => null, 'slug' => 'tenant_user'], [
            'name' => 'Tenant User', 'scope' => 'tenant', 'is_system' => true, 'is_active' => true,
        ]);

        User::updateOrCreate(['email' => env('DANUM_TENANT_USER_EMAIL', 'ucok@danum.local')], [
            'name' => 'Tenant User', 'nip' => null, 'email_verified_at' => now(),
