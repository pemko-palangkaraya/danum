...
class LetterTypeVersionManagementTest extends TestCase
{
    ...
    public function test_super_admin_can_open_version_management(): void
    {
        ...
        LetterTypeVersion::factory()->create(['letter_type_id' => $letterType->id, 'version' => 1, 'body_template' => 'v1', 'effective_from' => now()->subDay()]);
        $this->actingAs($admin)->get(route('letter-types.versions', $letterType))->assertOk();
    }
    ...
    public function test_tenant_user_cannot_open_version_management(): void
    {
        ...
        $this->actingAs($user)->get(route('letter-types.versions', $letterType))->assertForbidden();
    }
    ...
    public function test_creating_a_version_preserves_previous_version_and_closes_its_period(): void
    {
        ...
        $v1 = LetterTypeVersion::factory()->create([
            'letter_type_id' => $letterType->id, 'version' => 1, 'body_template' => 'body-v1',
            'template_path' => 'letter-templates/v1.docx', 'effective_from' => $start,
        ]);
        $effectiveFrom = now()->addMinute()->startOfSecond();
        $v2 = app(LetterTypeService::class)->createVersion($letterType, [
            'body_template' => 'body-v2', 'template_path' => 'letter-templates/v2.docx',
            'effective_from' => $effectiveFrom, 'change_note' => 'Penyesuaian format surat.',
        ], $admin->id);
    ...
    {
        $letterType = LetterType::factory()->create(['tenant_id' => null, 'status' => 'active', 'body_template' => 'body-v1', 'template_path' => 'letter-templates/v1.docx']);
        $v1 = LetterTypeVersion::factory()->create(['letter_type_id' => $letterType->id, 'version' => 1, 'body_template' => 'body-v1', 'template_path' => 'letter-templates/v1.docx', 'effective_from' => now()->subDay(), 'effective_until' => now()->addDay()]);
        $v2 = LetterTypeVersion::factory()->create(['letter_type_id' => $letterType->id, 'version' => 2, 'body_template' => 'body-v2', 'template_path' => 'letter-templates/v2.docx', 'effective_from' => now()->addDay()]);
    ...
    public function test_version_creation_is_audited(): void
    {
        ...
        LetterTypeVersion::factory()->create(['letter_type_id' => $letterType->id, 'version' => 1, 'effective_from' => now()->subDay()]);
        $this->actingAs($admin);
        $version = app(LetterTypeService::class)->createVersion($letterType, ['body_template' => 'v2', 'template_path' => 'letter-templates/v2.docx', 'effective_from' => now()->addMinute(), 'change_note' => 'Perubahan format.'], $admin->id);
        $this->assertDatabaseHas('audit_logs', ['user_id' => $admin->id, 'action' => 'letter_type.version.created', 'auditable_id' => $version->id]);
    }
    ...
    public function test_version_start_cannot_go_backwards(): void
    {
        ...
        app(LetterTypeService::class)->createVersion($letterType, ['body_template' => 'v2', 'effective_from' => now()->subDay(), 'change_note' => 'Invalid.'], $admin->id);
    }
}
