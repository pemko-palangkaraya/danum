...
class OutgoingLetterAuditLogTest extends TestCase
{
    ...
    public function test_create_update_submit_validate_and_issue_are_audited(): void
    {
        ...
        $this->actingAs($actor);
        $source = $letter->only(['tenant_id','created_by','letter_type_id','letter_type_version_id','validator_user_id','signer_user_id','number','recipient_name','recipient_address','subject','content','status']);
        $source['number'] = fake()->unique()->bothify('###/AUDIT/####');
        $created = $service->create($source, $actor->id);
        $created->update(['signer_position_id' => $position->id, 'signer_user_id' => $actor->id, 'generated_docx_path' => 'outgoing-letters/test.docx']);
        $service->update($created, ['subject' => 'Subjek Audit Baru']);
        $created->refresh();
        $workflow->submit($created, $actor->id);
        $created->refresh();
        ...
        $created->refresh();
        $this->assertSame('Saya telah memeriksa isi dan kelengkapan surat.', $created->verification_note);
    ...
    {
        $actor = User::factory()->superAdmin()->create();
        $letter = OutgoingLetter::factory()->create(['status' => OutgoingLetterStatus::DRAFT]);
        app(OutgoingLetterWorkflowService::class)->cancel($letter, $actor->id);
    ...
    public function test_rejection_and_withdrawal_decision_are_audited_without_exposing_verification_token(): void
    {
        ...
        $service = app(OutgoingLetterService::class);
        $request = $service->requestWithdrawal($letter, $requester->id, 'Surat perlu ditarik.', 'withdrawals/statement.pdf');
        $service->approveWithdrawal($request, $decider->id, 'Disetujui.');
        $logs = AuditLog::query()->where('auditable_type', OutgoingLetter::class)->where('auditable_id', $letter->id)->orderBy('created_at')->get();
        $this->assertSame(['outgoing_letter.withdrawal_requested','outgoing_letter.withdrawn'], $logs->pluck('action')->all());
