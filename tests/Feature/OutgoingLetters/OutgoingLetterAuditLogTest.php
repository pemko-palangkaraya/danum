...
class OutgoingLetterAuditLogTest extends TestCase
{
    ...
    {
        $actor = User::factory()->superAdmin()->create(); $validator = User::factory()->superAdmin()->create(); $letter = OutgoingLetter::factory()->create(['status' => OutgoingLetterStatus::DRAFT, 'validator_user_id' => $validator->id, 'signer_user_id' => $actor->id]); $service = app(OutgoingLetterService::class); $this->actingAs($actor);
