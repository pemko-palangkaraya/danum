...
class OutgoingLetterWorkflowTest extends TestCase
{
    ...
    public function test_create_records_draft_and_history(): void
    {
        ...
        $data = $source->only(['tenant_id', 'created_by', 'letter_type_id', 'number', 'recipient_name', 'recipient_address', 'subject', 'content', 'issued_at', 'status']);
        $data['number'] = fake()->unique()->bothify('###/SK/####');
        $created = app(OutgoingLetterService::class)->create($data, $user->id);
    ...
    public function test_submit_validate_issue_workflow_sets_active_letter_and_history(): void
    {
        ...
        try {
            ...
            $type = LetterType::factory()->create(['status' => LetterTypeStatus::ACTIVE, 'validity_period' => '6_months', 'has_expiry' => true]);
            $letter = OutgoingLetter::factory()->create(['letter_type_id' => $type->id, 'validator_user_id' => $validator->id, 'signer_user_id' => $user->id]);
            $this->prepareSignerCredentials($letter, $user);
            $workflow = app(OutgoingLetterWorkflowService::class);
            $issuance = app(OutgoingLetterService::class);
            $workflow->submit($letter, $user->id);
            $letter->refresh();
            $this->assertNotNull($letter->submitted_at);
            $workflow->validate($letter, $validator->id, 'Saya telah memeriksa kelengkapan dan kesesuaian surat.');
            $letter->refresh();
    ...
    public function test_submit_validate_and_issue_require_notes(): void
    {
        $user = User::factory()->superAdmin()->create();
        $validator = User::factory()->superAdmin()->create();
        $letter = OutgoingLetter::factory()->create(['validator_user_id' => $validator->id, 'signer_user_id' => $user->id]);
        $workflow = app(OutgoingLetterWorkflowService::class);
        $workflow->submit($letter, $user->id);
        $letter->refresh();
        try {
            $workflow->validate($letter, $validator->id);
            $this->fail('Verification without note should fail.');
        } catch (\DomainException $exception) {
            $this->assertSame('Catatan verifikasi wajib diisi.', $exception->getMessage());
        }
        $workflow->validate($letter, $validator->id, 'Verifikasi lengkap.');
        $letter->refresh();
        try {
            app(OutgoingLetterService::class)->issue($letter, $user->id);
            $this->fail('Signing without note should fail.');
        } catch (\DomainException $exception) {
            $this->assertSame('Catatan penandatanganan wajib diisi.', $exception->getMessage());
        }
    }
    ...
    public function test_submitted_draft_cannot_be_edited_or_deleted(): void
    {
        ...
        app(OutgoingLetterService::class)->update($letter, ['subject' => 'Tidak boleh']);
    }
    ...
    public function test_reject_returns_letter_to_draft_with_reason(): void
    {
        ...
        app(OutgoingLetterWorkflowService::class)->reject($letter, $user->id, 'Data penerima perlu diperbaiki.');
        $letter->refresh();
    ...
    public function test_editable_draft_can_be_cancelled(): void
    {
        ...
        app(OutgoingLetterWorkflowService::class)->cancel($letter, $user->id);
        $letter->refresh();
        $this->assertSame(OutgoingLetterStatus::CANCELLED, $letter->status);
        $this->assertDatabaseHas('outgoing_letter_status_histories', ['outgoing_letter_id' => $letter->id, 'action' => 'cancelled']);
    }
    ...
    {
        return ['none' => ['none', ''], 'one week' => ['1_week', '2026-09-01 10:20:00'], 'two weeks' => ['2_weeks', '2026-09-08 10:20:00'], 'one month' => ['1_month', '2026-09-25 10:20:00'], 'three months' => ['3_months', '2026-11-25 10:20:00'], 'six months' => ['6_months', '2027-02-25 10:20:00'], 'one year' => ['1_year', '2027-08-25 10:20:00']];
    }
    ...
    public function test_expired_letter_is_not_active(): void
    {
        ...
        try {
            $letter = OutgoingLetter::factory()->create(['status' => OutgoingLetterStatus::ISSUED, 'valid_from' => Carbon::parse('2026-08-25 10:20:00', 'Asia/Pontianak'), 'valid_until' => Carbon::parse('2026-09-01 10:20:00', 'Asia/Pontianak')]);
    ...
    public function test_withdrawal_request_requires_issued_letter_and_pending_request_blocks_duplicate(): void
    {
        ...
        $this->assertDatabaseHas('outgoing_letter_withdrawal_requests', ['id' => $request->id, 'outgoing_letter_id' => $letter->id, 'status' => OutgoingLetterWithdrawalStatus::PENDING->value]);
        $this->expectException(\DomainException::class);
