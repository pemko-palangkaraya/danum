...
class OutgoingLetterWorkflowGuardTest extends TestCase
{
    ...
    public function test_validation_changes_status_to_validated(): void
    {
        ...
        $letter = OutgoingLetter::factory()->create(['status' => OutgoingLetterStatus::DRAFT, 'submitted_at' => now(), 'validator_user_id' => $validator->id]);
    ...
    public function test_issue_requires_validated_status(): void
    {
        ...
        $letter = OutgoingLetter::factory()->create(['status' => OutgoingLetterStatus::DRAFT, 'signer_user_id' => $signer->id]);
        $this->expectException(\DomainException::class);
        app(OutgoingLetterService::class)->issue($letter, $signer->id, 'Saya menyetujui penerbitan surat.', 'test-passphrase-123');
    }
    ...
    public function test_issue_requires_assigned_signer(): void
    {
        $signer = User::factory()->superAdmin()->create();
        $otherUser = User::factory()->superAdmin()->create();
        $letter = OutgoingLetter::factory()->create(['status' => OutgoingLetterStatus::VALIDATED, 'signer_user_id' => $signer->id]);
        $this->expectException(\DomainException::class);
        app(OutgoingLetterService::class)->issue($letter, $otherUser->id, 'Saya menyetujui penerbitan surat.', 'test-passphrase-123');
    }
    ...
    public function test_assigned_signer_can_issue_validated_letter_with_note(): void
    {
        ...
        app(OutgoingLetterService::class)->issue($letter->fresh(), $signer->id, 'Saya menyetujui penerbitan surat.', 'test-passphrase-123');
        $letter->refresh();
    ...
    private function prepareSignerCredentials(OutgoingLetter $letter, User $signer): void
    {
        ...
        Storage::disk('local')->put('outgoing-letters/test/source.docx', 'test docx');
