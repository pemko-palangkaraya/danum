...
class CitizenDeathIssuanceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->mock(DocxTteService::class, function ($mock): void {
            $mock->shouldReceive('createIssuedCopy')->andReturn('outgoing-letters/test/issued.docx');
        });
    ...
    public function test_issuing_death_letter_updates_citizen_and_removes_active_family_membership(): void
    {
        ...
        $citizen = Citizen::factory()->forTenant($tenant)->create(['tanggal_lahir' => '1980-01-10', 'status_kependudukan' => 'aktif', 'tanggal_meninggal' => null]);
        $family = Family::factory()->forTenant($tenant)->create(['head_citizen_id' => $citizen->id]);
        $membership = FamilyMember::factory()->forFamily($family)->forCitizen($citizen)->relation('head')->create();
        $type = LetterType::factory()->create(['tenant_id' => $tenant->id, 'code' => 'sk_kematian', 'status' => LetterTypeStatus::ACTIVE]);
        ...
        $this->assertDatabaseHas('population_events', ['tenant_id' => $tenant->id, 'citizen_id' => $citizen->id, 'event_type' => 'death', 'event_date' => '2025-12-12', 'effective_date' => '2025-12-12', 'document_number' => $letter->number, 'created_by' => $letter->created_by]);
    }
